...
- Konkurrenz Offer-Create (korrigiert 2026-09-07, belegt durch Race-Tests in `tests/db/f202-variant-deepening.test.ts`): Auf Service-Ebene serialisiert der Projekt-Lock (`FOR UPDATE`). Identisches Material → Digest-Replay: beide Aufrufer erhalten dasselbe Offer (kein Fehler, kein Duplikat). Divergierendes Material → der Verlierer erhält `OfferConflictError` (Fehler an den Client, kein automatisches Retry, kein Duplikat). `OfferIntegrityError` bleibt letzte Schranke ausschließlich für Lock-umgehende Schreiber (partieller Index, per direktem DB-Write belegt) — kein Service-Pfad erzeugt ihn.
...
- Konkurrenz-Verlierer: Divergierendes Material → `OfferConflictError` an den Client („Offer inzwischen mit anderem Stand angelegt, erneut laden"), kein automatisches Retry. Identisches Material → idempotentes Replay (kein Fehler). Gegenläufige `setPrimaryVariant`-Aufrufe: Last-Writer-Wins, invariant exakt eine Primary (Race-belegt).
...
## Tests (RED zuerst)
- DB/Contract: Exactly-One (zweite Primary verletzt Index), Promote-Switch, Demote-Ablehnung (kein API-Pfad), Erstvariante primär, Duplikat/Resolution-Basis nicht primär, Override-Range/Null + Audit-Event ohne Revisionsbump, Bundles-Form/Positions-Eindeutigkeit (leeres name, Array-Max), Backfill-Verifikation (jedes Offer mit ≥1 Variante hat genau eine Primary), konkurrierende `setPrimaryVariant`-Aufrufe (gleiches Ziel → genau ein Event; gegenläufig → exakt eine Primary; Index als letzte Schranke per Direkt-Write), konkurrierende Offer-Creates (identisch → idempotentes Replay ohne Duplikat; divergierend → Verlierer `OfferConflictError`), Anzeige-Fallback (Override aktiv → nur Netto definiert; gecleart; nach Primary-Switch; ohne Primary → Totals null), Cross-Offer-/Cross-Tenant-Promote → `OfferNotFoundError`, Idempotenz (No-op ohne Event), Audit-Events je Mutation.
