...
- Audit-Events (bestehendes `emitEvent`-Muster, Payload `{offerId, variantId?, actor, at}`): `offer.primary_switched`, `offer.total_override_set` (+ `valueNetCents`, damit der Audit-Trail den Wert rekonstruiert), `offer.total_override_cleared` (ohne Wert), `offer.variant_bundles_set`. Identisches Bundle-Array → idempotentes No-op ohne Event (No-op-Disziplin wie Primary-Switch).
