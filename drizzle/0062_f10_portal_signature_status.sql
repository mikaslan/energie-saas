...
-- Owner-Fenster nach Vorlage 0016 (CI-Root-Cause 2026-09-05): Die Funktion
-- gehört seit 0056 app_owner, das SET-Fenster ist zu (`set false`). Wer sie
-- ersetzt, verschafft sich SET temporär und gibt es zurück. Body unverändert
-- — Rollen-Pin 6d025bff bleibt gültig. Im Strict-Modus (Session läuft
-- bereits als app_owner) entfällt der Wechsel per Guard.
DO $f1001_owner_repair$
DECLARE
...
BEGIN
  IF v_app <> 'app_owner' THEN
    ...
    -- Definer-Rechte für den Signatur-Join (Review-Fund 2026-09-05:
  ...
  IF v_switched THEN
    ...
    EXECUTE pg_catalog.format('GRANT app_owner TO %I WITH inherit false, set false', v_app);
  END IF;
END
$f1001_owner_repair$;--> statement-breakpoint
