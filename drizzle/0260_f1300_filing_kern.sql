ALTER TABLE "subsidy_case" DROP CONSTRAINT "subsidy_case_status_ck";--> statement-breakpoint
ALTER TABLE "subsidy_case" ALTER COLUMN "status" SET DEFAULT 'draft';--> statement-breakpoint
ALTER TABLE "file_request" ADD COLUMN "slot_type" text;--> statement-breakpoint
ALTER TABLE "file_request" ADD CONSTRAINT "file_request_slot_type_ck" CHECK ("file_request"."slot_type" is null or "file_request"."slot_type" in (
        'bza_angebot', 'bza_vollmacht', 'bnd_rechnung', 'typenschild_foto'
      ));--> statement-breakpoint
ALTER TABLE "subsidy_case" ADD CONSTRAINT "subsidy_case_status_ck" CHECK ("subsidy_case"."status" in (
        'draft', 'vorbereitung', 'bza_eingereicht', 'korrektur', 'bza_bewilligt',
        'bnd_eingereicht', 'abgeschlossen', 'storniert'
      ));
--> statement-breakpoint
-- =======================================================================
-- F13-00 Filing-Kern (Resolver-Amendment): Der Portal-DEFINER blendet
-- Foerderakten im Status draft aus (F13-00 §1: Entwurf ist unsichtbar
-- fuer Externe — kein Portal-Statuswort draft, Resolver projiziert
-- subsidy = null; Chat faellt mit, da nur an subsidy_entry gehaengt).
-- Ansonsten byte-identische Uebernahme von 0135 (beide Rollen-Pfade).
-- =======================================================================
DO $f1300_replace_resolver$
DECLARE
...
$f1300_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1300_replace_resolver$;--> statement-breakpoint
DO $f1300_grants$
BEGIN
