CREATE TABLE "file_request" (
	...
	CONSTRAINT "file_request_status_ck" CHECK ("file_request"."status" in (
        'offen', 'hochgeladen', 'erledigt', 'storniert'
      )),
	CONSTRAINT "file_request_title_ck" CHECK (pg_catalog.length(pg_catalog.btrim("file_request"."title")) between 1 and 160),
	CONSTRAINT "file_request_description_ck" CHECK ("file_request"."description" is null or pg_catalog.length("file_request"."description") between 1 and 2000),
	CONSTRAINT "file_request_receipt_ck" CHECK (
      case when "file_request"."status" in ('hochgeladen', 'erledigt')
        then "file_request"."storage_key" is not null
         and "file_request"."file_sha256" is not null
         and "file_request"."content_type" is not null
         and "file_request"."byte_size" is not null
         and "file_request"."original_filename" is not null
         and "file_request"."uploaded_at" is not null
      else "file_request"."storage_key" is null
       and "file_request"."file_sha256" is null
       and "file_request"."content_type" is null
       and "file_request"."byte_size" is null
       and "file_request"."original_filename" is null
       and "file_request"."uploaded_at" is null
      end
    ),
	CONSTRAINT "file_request_byte_size_ck" CHECK ("file_request"."byte_size" is null or ("file_request"."byte_size" between 1 and 10485760))
);--> statement-breakpoint
ALTER TABLE "file_request" ADD CONSTRAINT "file_request_workspace_id_fk" FOREIGN KEY ("workspace_id") REFERENCES "public"."workspace"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "file_request" ADD CONSTRAINT "file_request_project_fk" FOREIGN KEY ("workspace_id","project_id") REFERENCES "public"."project"("workspace_id","id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "file_request" ADD CONSTRAINT "file_request_created_by_fk" FOREIGN KEY ("workspace_id","created_by") REFERENCES "public"."membership"("workspace_id","user_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
CREATE UNIQUE INDEX "file_request_ws_id_uq" ON "file_request" USING btree ("workspace_id","id");--> statement-breakpoint
CREATE INDEX "file_request_ws_project_idx" ON "file_request" USING btree ("workspace_id","project_id","status");--> statement-breakpoint
ALTER TABLE public.file_request ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE public.file_request FORCE ROW LEVEL SECURITY;--> statement-breakpoint
CREATE POLICY tenant_isolation ON public.file_request
...
-- (Titel/Beschreibung pflegbar, v1 ein Beleg je Anfrage). Maschine
-- offen → hochgeladen (Kunde via Token-DEFINER) → erledigt; storniert
-- nur aus offen (logisch, nie DELETE). Beleg unter immutable/-WORM-Key,
-- Empfangs-QR nur intern (Key/Pruefsumme nie ins Portal projiziert).
-- RLS-Vertrag tenant_isolation + FORCE (Muster 0103). Rechte:
-- project.read/project.write im Service-Layer (keine neue Permission,
-- keine Grants — Rollenvertrag wie 0103).
-- ═══════════════════════════════════════════════════════════════════════

-- F10-04 Token-Erfüllung (erster anonymer Schreibpfad des Portals):
-- EIN atomares UPDATE (offen → hochgeladen) mit Token-Invite-Bindung
-- (Mandant + Projekt aus dem Invite, nie aus dem Request). 'conflict'
-- bei bereits erfüllter/stornierter Anfrage, 'not_found' ohne Orakel
-- (unbekannt/deformiert/entzogen/abgelaufen/fremd). Der Storage-Put
-- läuft VORHER im Service; bei 'conflict' bleibt ein verwaistes
-- WORM-Objekt liegen (unveraenderlich, unreferenziert — ESTIMATE,
-- kein Object-Lock-Cleanup in v1).
CREATE FUNCTION public.fulfill_file_request(
...
BEGIN
  ...
  SELECT true INTO request_exists
  ...
  RETURN 'not_found';
END
$f1004_fulfill$;--> statement-breakpoint
REVOKE ALL ON FUNCTION public.fulfill_file_request(bytea, uuid, text, text, text, integer, text) FROM PUBLIC;--> statement-breakpoint
-- Owner-Tanz NUR im Testmodus (Muster 0064 — dort sign_by_token mit
-- identischem Befund: RESTRICTIVE-Actor-Policies tragen den
-- app_owner-Escape-Hatch, der nur greift, wenn CURRENT_USER =
-- 'app_owner' ist; Strict migriert ohnehin als app_owner).
DO $f1004_fulfill_owner_dance$
DECLARE
...
BEGIN
  IF v_app <> 'app_owner' THEN
    ...
    ALTER FUNCTION public.fulfill_file_request(
      bytea, uuid, text, text, text, integer, text
    ) OWNER TO app_owner;
    REVOKE CREATE ON SCHEMA public FROM app_owner;
    SET ROLE app_owner;
    REVOKE ALL ON FUNCTION public.fulfill_file_request(
      bytea, uuid, text, text, text, integer, text
    ) FROM PUBLIC;
    EXECUTE pg_catalog.format(
      'GRANT EXECUTE ON FUNCTION public.fulfill_file_request(
        bytea, uuid, text, text, text, integer, text
      ) TO %I',
      v_app
    );
  ...
  END IF;
END
$f1004_fulfill_owner_dance$;--> statement-breakpoint

-- ═══════════════════════════════════════════════════════════════════════
-- F10-04 Datei-Anfragen: resolve_portal_public_view projiziert
-- zusaetzlich fileRequests (offen + hochgeladen: id/Titel/Beschreibung/
-- Stand/Zeiten/Dateiname — nie Storage-Key/Pruefsumme).
-- Funktions-Pin im Rollenvertrag nachgezogen (Hash aus diesem Body).

-- Owner-sicherer Funktionsersatz (Muster 0091/0059/0056).
DO $f1004_replace_resolver$
DECLARE
  v_app name := current_user;
BEGIN
  IF v_app = 'app_owner' THEN
...
$f1001_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1004_replace_resolver$;--> statement-breakpoint
...
BEGIN
  ...
  END IF;
END
$f1004_grants$;--> statement-breakpoint

DO $f1004_fulfill_acl$
BEGIN
  ...
  END IF;
END
$f1004_fulfill_acl$;--> statement-breakpoint
