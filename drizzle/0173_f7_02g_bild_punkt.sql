-- F7-02G: Bild-Punkt (Katalog F7.2). Vollkopie von 0144 plus Foto:
-- kind='image' (Arbeitspunkt wie task: required/done erlaubt) und
-- optionaler Foto-Key `photo` nur dort (Spiegel-Regel zu value, F7-02E-
-- Praezedenz). Der Key ist projekt-skoped
-- (immutable/<projekt>/checklist-photos/<punkt>_<sha8>.<ext>)
-- und wird nur vom Service gebaut; die Strukturfunktion streicht `photo`
-- wie `done`/`value` (Editor-Antwort auf Vorlagen ab Version 1, F7-03b-
-- Praezedenz). Keine neue Tabelle, keine neue Permission (checklist.write),
-- keine RLS-Aenderung.
