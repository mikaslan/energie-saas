-- M1-07: Technische Fehler VOR einem erfolgreich committed Domain-Claim
-- muessen von pg-boss erneut zugestellt werden. Diese Retries besitzen keinen
-- fachlichen attempt_count; der steigt weiterhin ausschliesslich im Domain-CAS.
DO $m1_07_technical_retry_migration$
DECLARE
  pgboss_owner text;
...
BEGIN
  ...
  IF pgboss_owner IS NULL THEN
    IF CURRENT_USER = SESSION_USER
      ...
      RETURN;
    END IF;
    RAISE EXCEPTION 'M1-07 technical retry: pgboss-Schema fehlt';
  END IF;
  IF pgboss_owner <> 'app_worker' THEN
    RAISE EXCEPTION 'M1-07 technical retry: pgboss muss app_worker gehoeren';
  END IF;
  IF NOT pg_catalog.pg_has_role(SESSION_USER, 'app_worker', 'SET') THEN
    RAISE EXCEPTION 'M1-07 technical retry: app_migrator braucht SET-only zu app_worker';
  END IF;
  ...
  IF pgboss_version IS DISTINCT FROM 38 THEN
    RAISE EXCEPTION 'M1-07 technical retry: erwartet pg-boss v38, ist %', pgboss_version;
  END IF;
  ...
  UPDATE pgboss.queue
     SET retry_limit = 10,
         ...
         retry_backoff = true,
         retry_delay_max = 60,
         updated_on = pg_catalog.now()
   WHERE name = 'calculation.execute'
     AND policy = 'exclusive'
     AND notify = false;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'M1-07 technical retry: calculation.execute-Queue fehlt oder driftet';
  END IF;

  -- Bereits wartende Zustellungen erhalten denselben technischen Vertrag.
  UPDATE pgboss.job
     SET retry_limit = 10,
         retry_delay = 1,
         retry_backoff = true,
  ...
  EXECUTE $technical_retry_ddl$
    ...
    BEGIN
      ...
      SELECT queue_config.name, dispatch_payload, 0, dispatch_start_after,
             dispatch_key, queue_config.expire_seconds,
             queue_config.deletion_seconds,
             dispatch_start_after
               + queue_config.retention_seconds * interval '1 second',
             queue_config.retry_limit, queue_config.retry_delay,
             queue_config.retry_backoff, queue_config.retry_delay_max,
             queue_config.policy, queue_config.dead_letter,
...
END
$m1_07_technical_retry_migration$;
