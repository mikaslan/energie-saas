-- ═══════════════════════════════════════════════════════════════════════
-- F10-15 KfW-Upload-Kontext (Katalog F10.2 „mit Chat + Upload-Dropzone"):
-- Nur Resolver-Replace (kein Modell — subsidy_case_id existiert seit
-- F13-07): fileRequests[].subsidyLinked ((subsidy_case_id IS NOT NULL),
-- minimiertes Bit, nie die Akten-ID). Replace laeuft im Owner-Tanz
-- (Muster 0170). Rechte: keine neue Permission, keine Grants.
-- ═══════════════════════════════════════════════════════════════════════
DO $f1015_replace_resolver$
DECLARE
...
$f1015_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1015_replace_resolver$;--> statement-breakpoint
