-- ═══════════════════════════════════════════════════════════════════════
-- F10-16 Gegenzeichnung in der Portal-Timeline (Katalog F7.7/F10.2):
-- Nur Resolver-Replace (0172-Vollkopie): Timeline-Allowlist + 4. Typ
-- installation.handover_countersigned → handover_countersigned (CASE +
-- IN-Liste, beide Owner-Tanz-Ruempfe). Event existiert seit F7-07b (0176),
-- Payload PII-frei (nur projectId); Projektion weiter nur Typ+Zeit+Tag.
-- Replace laeuft im Owner-Tanz (Muster 0172).
-- Rechte: keine neue Permission, keine Grants.
-- ═══════════════════════════════════════════════════════════════════════
DO $f1016_replace_resolver$
DECLARE
...
$f1016_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1016_replace_resolver$;--> statement-breakpoint
