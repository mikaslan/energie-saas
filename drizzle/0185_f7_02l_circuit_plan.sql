-- F7-02L: Schaltplan-Punkt (Katalog F7.2: circuit-plan als Anzeige-Punkt
-- wie title/description — weder Pflicht noch abhakbar, keine Nutzlast).
-- Vollkopie von 0180 plus kind='circuit-plan' (kind-IN und Anzeige-Regel).
-- Der Tree speichert nur die Art (kein Inhalt, kein Snapshot — 03B „Art
-- nie Inhalt"); die Anzeige rendert den F7-11-Einlinien-Schaltplan der
-- gebundenen Variante read-only (installation.read-gated). Keine neue
-- Tabelle, keine neue Permission (checklist.write), keine RLS-Aenderung.
-- (Zeilen 9+ wortgleich zu 0180 bis auf die zwei kind-Stellen.)
    ...
    LOOP
      ...
      LOOP
        ...
        IF pg_catalog.jsonb_typeof(item_record.value) <> 'object'
           ...
           OR item_record.value - ARRAY['id','title','done','required','visible','irrelevant','visibleIf','kind','description','value','photo','photos','signerRole','componentId']::text[]
              <> '{}'::jsonb
           ...
           OR (item_record.value ? 'kind'
               ...
               AND NOT (item_record.value->>'kind' IN ('task','title','description','radio','text','multi','image','signature','component-list','datasheets','circuit-plan')))
           ...
           -- F7-15 (0180): optionale Foto-Galerie photos (nur Bildpunkt).
           -- Fehlender Key und explizites null sind gueltig (kein Batch).
           -- Non-empty photos bindet das Cover: photo non-null UND
           -- photo = photos->>0; jedes Element traegt dasselbe Key-Format
           -- wie photo (Service-seitig gebaut, s. 02G-Kommentar oben).
           OR (item_record.value ? 'photos'
               AND (item_record.value->'photos') IS NOT NULL
               AND (item_record.value->'photos') <> 'null'::jsonb
               AND (pg_catalog.jsonb_typeof(item_record.value->'photos') <> 'array'
                    OR pg_catalog.jsonb_array_length(item_record.value->'photos') < 1
                    OR pg_catalog.jsonb_array_length(item_record.value->'photos') > 8))
           OR ((item_record.value ? 'photos')
               AND (item_record.value->'photos') IS NOT NULL
               AND (item_record.value->'photos') <> 'null'::jsonb
               AND EXISTS (
                 SELECT 1
                   FROM pg_catalog.jsonb_array_elements(item_record.value->'photos') AS element(value)
                  WHERE pg_catalog.jsonb_typeof(element.value) <> 'string'
                     OR NOT public._f704_valid_clean_text(element.value #>> '{}', 500)
                     OR (element.value #>> '{}') !~
                        '^immutable/[0-9a-f-]{36}/checklist-photos/[0-9a-f-]{36}_[0-9a-f]{8}\.(jpg|jpeg|png)$'))
           OR ((item_record.value ? 'photos')
               AND (item_record.value->'photos') IS NOT NULL
           ...
           OR ((item_record.value ? 'photos')
               AND (item_record.value->'photos') IS NOT NULL
               AND (item_record.value->'photos') <> 'null'::jsonb
               AND ((item_record.value->'photo') IS NULL
                    OR (item_record.value->'photo') = 'null'::jsonb
                    OR (item_record.value->>'photo') IS DISTINCT FROM (item_record.value->'photos'->>0)))
           ...
           OR ((item_record.value->>'kind' IN ('title','description','component-list','datasheets','circuit-plan'))
...
-- F7-15 (0180): zusaetzlich `photos` gestrichen (Galerie ist Nutzlast).
                ...
                item_record.value - CASE
                  WHEN (block_record.value->>'visible')::boolean
                   ...
                   AND (item_record.value->>'visible')::boolean
                    THEN ARRAY['done','value','photo','photos']
                  ELSE '{}'::text[]
                END
