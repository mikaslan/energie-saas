...
-- ═══════════════════════════════════════════════════════════════════════
-- F10.2: Funktionsersatz OHNE Owner-Verlust. Seit 0056 gehoert
-- resolve_portal_public_view app_owner; CREATE OR REPLACE verlangt aber
-- den aktuellen Owner ("must be owner of function"). Strict laeuft die
-- Migration ohnehin als app_owner (erster Zweig). Im Ein-Rollen-
-- Testmodus uebernimmt kurz SET ROLE app_owner (SET-Recht fuer genau
-- diesen Schritt) und fuehrt DIESELBE DDL aus. CREATE OR REPLACE
-- erhaelt Ownership und ACLs — der 0056-Zustand (Owner app_owner,
-- EXECUTE nur fuer die App-Rolle) bleibt also unveraendert bestehen.
-- Der Schluss-Block erteilt nur den neuen Projektion-Grant.
-- ═══════════════════════════════════════════════════════════════════════
