ALTER TABLE "file_request" ADD COLUMN "file_type" text DEFAULT 'any' NOT NULL;--> statement-breakpoint
ALTER TABLE "file_request" ADD CONSTRAINT "file_request_file_type_ck" CHECK ("file_request"."file_type" in ('any', 'pdf', 'image'));
-- ═══════════════════════════════════════════════════════════════════════
-- F10-13 Datei-Anfragen Dateityp (Katalog F10.2 „Templates: Titel,
-- Dateityp, Allow many"): Spalte file_type ('any'/'pdf'/'image', Bestand
-- 'any' = verhaltenserhaltend) auf file_request (Generator-Anteil oben)
-- und file_request_template (Hand-Anteil — Vorlage liegt nicht im
-- Drizzle-Schema). Resolver projiziert 'fileType' je Anfrage; beide
-- Fulfill-Kapseln verweigern Fehltypen mit 'invalid' (Defense in Depth,
-- Service prueft vorab typgenau). Alle drei Replaces laufen im
-- Owner-Tanz (Muster 0135: plain OR REPLACE scheitert im Testmodus mit
-- „must be owner"). Rechte: keine neue Permission, keine Grants
-- (Tabellen-Grants decken neue Spalten).
-- ═══════════════════════════════════════════════════════════════════════
ALTER TABLE "file_request_template" ADD COLUMN "file_type" text DEFAULT 'any' NOT NULL;--> statement-breakpoint
ALTER TABLE "file_request_template" ADD CONSTRAINT "file_request_template_file_type_ck" CHECK ("file_request_template"."file_type" in ('any', 'pdf', 'image'));--> statement-breakpoint
...
END
$f1013_resolve_token$;$ddl$;
    EXECUTE $ddl$CREATE OR REPLACE FUNCTION public.fulfill_file_request(
...
BEGIN
  ...
  -- F10-13: Dateityp-Guard (nur auf dem offenen Pfad; terminale
  -- Staende melden weiter 'conflict', unbekannte 'not_found' —
  -- kein Orakel ueber Typ/Stand. Service normalisiert vorab.)
  SELECT freq.status, freq.file_type
    INTO request_status, request_file_type
    FROM public.file_request AS freq
   WHERE freq.workspace_id = invite_row.workspace_id
     AND freq.project_id = invite_row.project_id
     AND freq.id = requested_request_id;
  IF FOUND AND request_status = 'offen' AND NOT (
    request_file_type = 'any'
    OR (request_file_type = 'pdf' AND p_content_type = 'application/pdf')
    OR (request_file_type = 'image'
        AND p_content_type IN ('image/jpeg', 'image/png'))
  ) THEN
    RETURN 'invalid';
  END IF;
...
$f1004_fulfill$;
$ddl$;
    EXECUTE $ddl$CREATE OR REPLACE FUNCTION public.fulfill_file_request_followup(
  requested_token_hash bytea,
  requested_request_id uuid,
  p_storage_key text,
  p_file_sha256 text,
...
BEGIN
  ...
  -- F10-13: Dateityp-Guard (Muster Erst-Kapsel; 'invalid' = Fehltyp
  -- auf sonst gueltigem Pfad).
  IF NOT (
    request_row.file_type = 'any'
    OR (request_row.file_type = 'pdf' AND p_content_type = 'application/pdf')
    OR (request_row.file_type = 'image'
        AND p_content_type IN ('image/jpeg', 'image/png'))
  ) THEN
    RETURN 'invalid';
  END IF;
