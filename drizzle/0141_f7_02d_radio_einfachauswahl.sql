...
-- Validator erlaubt `kind = 'radio'` je Punkt (abhakbar wie Aufgabe) und
-- erzwingt hoechstens einen erledigten Radio-Punkt je Segment
-- (Scope spiegelt die Segment-Lokalitaet der visibleIf-Regeln).
-- Keine neue Tabelle, keine neue Permission (checklist.write),
-- keine RLS-Aenderung.
      ...
      LOOP
        ...
        IF pg_catalog.jsonb_typeof(item_record.value) <> 'object'
           ...
           OR (item_record.value ? 'kind'
               ...
               AND NOT (item_record.value->>'kind' IN ('task','title','description','radio')))
