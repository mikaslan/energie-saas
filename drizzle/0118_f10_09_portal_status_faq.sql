CREATE TABLE "portal_status_faq" (
	...
	CONSTRAINT "portal_status_faq_scope_ck" CHECK ("portal_status_faq"."scope" = 'installation'),
	CONSTRAINT "portal_status_faq_key_ck" CHECK ("portal_status_faq"."source_key" in ('active', 'completed', 'handover')),
	CONSTRAINT "portal_status_faq_faq_ck" CHECK ("portal_status_faq"."faq" = pg_catalog.btrim("portal_status_faq"."faq") and pg_catalog.length("portal_status_faq"."faq") between 1 and 2000 and "portal_status_faq"."faq" !~ '[[:cntrl:]]'),
	CONSTRAINT "portal_status_faq_timestamps_ck" CHECK ("portal_status_faq"."updated_at" >= "portal_status_faq"."created_at" and pg_catalog.isfinite("portal_status_faq"."created_at") and pg_catalog.isfinite("portal_status_faq"."updated_at"))
);
--> statement-breakpoint
ALTER TABLE "portal_status_faq" ADD CONSTRAINT "portal_status_faq_workspace_id_fk" FOREIGN KEY ("workspace_id") REFERENCES "public"."workspace"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
CREATE UNIQUE INDEX "portal_status_faq_ws_id_uq" ON "portal_status_faq" USING btree ("workspace_id","id");--> statement-breakpoint
CREATE UNIQUE INDEX "portal_status_faq_ws_scope_key_uq" ON "portal_status_faq" USING btree ("workspace_id","scope","source_key");--> statement-breakpoint
-- ═══════════════════════════════════════════════════════════════
-- F10-09 Portal-FAQ je Installationsstand (Muster 0113): eine FAQ je
-- (Scope, Schlüssel); Resolver projiziert statusFaq. Reine
-- Leseprojektion des DEFINER-Resolvers; Schreibrechte
-- installation.read/write im Service (keine neue Permission).
-- ═══════════════════════════════════════════════════════════════
ALTER TABLE public.portal_status_faq ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE public.portal_status_faq FORCE ROW LEVEL SECURITY;

CREATE POLICY tenant_isolation ON public.portal_status_faq
  USING (workspace_id = NULLIF(pg_catalog.current_setting('app.workspace_id', true), '')::uuid)
  WITH CHECK (workspace_id = NULLIF(pg_catalog.current_setting('app.workspace_id', true), '')::uuid);
--> statement-breakpoint
DO $f1009_replace_resolver$
DECLARE
...
$f1009_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1009_replace_resolver$;--> statement-breakpoint
DO $f1009_grants$
BEGIN
