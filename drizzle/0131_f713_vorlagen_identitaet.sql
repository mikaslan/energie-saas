-- F7-13: Vorlagen-Identitaet componentId (Katalog F7.3, Merge/Reset).
-- Validator erlaubt den optionalen Key `componentId` (UUID-String) je Punkt
-- fuer das Merge-Matching; fehlender Key und explizites null sind gueltig
-- (Legacy-Bestand matcht per Titel). Keine neue Tabelle, keine neue
-- Permission (checklist.write/unlock), keine RLS-Aenderung.
    ...
    LOOP
      ...
      LOOP
        ...
        IF pg_catalog.jsonb_typeof(item_record.value) <> 'object'
           ...
           OR item_record.value - ARRAY['id','title','done','required','visible','irrelevant','visibleIf','kind','description','componentId']::text[]
              <> '{}'::jsonb
