-- ═══════════════════════════════════════════════════════════════════════
-- F13-06 Service-Sicht + Kundenbestaetigung im Kundenportal
-- (Katalog F13.2-Folge): Die Portal-Projektion zeigt je aktivem Invite
-- die Servicevorgaenge des Projekts (Titel/Stand/Zeiten/Bestaetigung,
-- nie description, nie cancelled). Zweiter anonymer Schreibpfad nach
-- F10-04: confirm_service_case bestaetigt erledigte Vorgaenge
-- (Invite-Bindung, uniform not_found, idempotent already).
-- Spalte service_case.confirmed_at + CHECK (nur an done). Reine
-- DEFINER-Pfade (Muster 0104/0106); keine neuen Rechte.
-- ═══════════════════════════════════════════════════════════════════════

ALTER TABLE public.service_case
  ADD COLUMN confirmed_at timestamp with time zone;--> statement-breakpoint
ALTER TABLE public.service_case
  ADD CONSTRAINT service_case_confirmed_ck
  CHECK (confirmed_at IS NULL OR status = 'done');--> statement-breakpoint
DO $f1007_replace_resolver$
DECLARE
...
$f1001_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1007_replace_resolver$;--> statement-breakpoint
...
BEGIN
  ...
  END IF;
END
$f1007_grants$;--> statement-breakpoint
-- F13-06 Kundenbestaetigung (zweiter anonymer Schreibpfad des
-- Portals, Muster fulfill_file_request): EIN atomares UPDATE
-- (done+unbestaetigt → confirmed_at) mit Token-Invite-Bindung
-- (Mandant + Projekt aus dem Invite, nie aus dem Request). 'already'
-- bei bereits bestaetigtem done (idempotent, kein Fehler),
-- 'not_found' uniform ohne Orakel (unbekannt/deformiert/entzogen/
-- abgelaufen/fremd/nicht-done — kein Rueckschluss auf Existenz/Stand).
CREATE FUNCTION public.confirm_service_case(
...
BEGIN
  ...
  UPDATE public.service_case AS svc_case
     SET confirmed_at = mutation_time
   WHERE svc_case.workspace_id = invite_row.workspace_id
     AND svc_case.project_id = invite_row.project_id
     AND svc_case.id = requested_case_id
     AND svc_case.status = 'done'
     AND svc_case.confirmed_at IS NULL;
  ...
  IF updated_count = 1 THEN
    INSERT INTO public.domain_events (
      ...
      invite_row.workspace_id, 'project', invite_row.project_id,
      'service_case.confirmed', 'system',
      pg_catalog.jsonb_build_object(
        'caseId', requested_case_id,
        'inviteId', invite_row.id
      ),
      mutation_time
    );
  ...
  SELECT true INTO case_confirmed
    FROM public.service_case AS svc_case
   WHERE svc_case.workspace_id = invite_row.workspace_id
     AND svc_case.project_id = invite_row.project_id
     AND svc_case.id = requested_case_id
     AND svc_case.status = 'done'
     AND svc_case.confirmed_at IS NOT NULL;
  IF case_confirmed THEN
    RETURN 'already';
  END IF;
  RETURN 'not_found';
END
$f1306_confirm$;--> statement-breakpoint
REVOKE ALL ON FUNCTION public.confirm_service_case(bytea, uuid) FROM PUBLIC;--> statement-breakpoint
...
BEGIN
  IF v_app <> 'app_owner' THEN
    ...
    GRANT CREATE ON SCHEMA public TO app_owner;
    ALTER FUNCTION public.confirm_service_case(
      bytea, uuid
    ) OWNER TO app_owner;
    REVOKE CREATE ON SCHEMA public FROM app_owner;
    SET ROLE app_owner;
    REVOKE ALL ON FUNCTION public.confirm_service_case(
      bytea, uuid
    ) FROM PUBLIC;
    EXECUTE pg_catalog.format(
      'GRANT EXECUTE ON FUNCTION public.confirm_service_case(
        bytea, uuid
      ) TO %I',
      v_app
    );
    ...
    GRANT SELECT, UPDATE ON public.service_case TO app_owner;
    GRANT SELECT, INSERT ON public.domain_events TO app_owner;
  END IF;
END
$f1306_confirm_owner_dance$;--> statement-breakpoint

-- Oeffentliche Ausfuehrung rollenunabhaengig (Muster f1004_fulfill_acl):
...
BEGIN
  ...
  END IF;
END
$f1306_confirm_acl$;--> statement-breakpoint
