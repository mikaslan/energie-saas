ALTER TABLE "portal_status_label" ADD COLUMN "visible" boolean DEFAULT true NOT NULL;
ALTER TABLE "portal_status_label" ALTER COLUMN "label" DROP NOT NULL;--> statement-breakpoint
-- ═══════════════════════════════════════════════════════════════════════
-- F10-14 Installations-Sichtbarkeit (Katalog F10.2 „+ Sichtbarkeit"):
-- Spalte visible (BOOLEAN NOT NULL DEFAULT true, Bestand sichtbar =
-- verhaltenserhaltend) auf portal_status_label (Generator-Anteil oben);
-- label wird NULLABLE (NULL = reine Sichtbarkeits-Zeile, damit
-- uebersetzte Portal-Fallbacks je Sprache greifen statt deutschem
-- Default-Text). Resolver projiziert installation.statusVisibility je
-- Zeile (source_key → visible, ungemappt = {}) und filtert NULL-Labels
-- aus statusLabels. Replace laeuft im Owner-Tanz (Muster 0170).
-- Rechte: keine neue Permission, keine Grants.
-- ═══════════════════════════════════════════════════════════════════════
DO $f1014_replace_resolver$
DECLARE
...
$f1014_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1014_replace_resolver$;--> statement-breakpoint
