-- ═══════════════════════════════════════════════════════════════════════
-- F10-17 Portal My-Files (Katalog F10.2): Kunden-Sichtbarkeit je
-- Projekt-Datei (visible_to_customer, DEFAULT false) + Resolver-
-- Projektion (0179-Vollkopie: project_files_list, nur sichtbare,
-- newest-first, nie Key/Hash) + Download-Kapsel
-- read_portal_project_file_artifact (F10-07/0116-Muster, Sichtbarkeits-
-- WHERE in der Funktion). Kein Loeschen, keine neue Permission, kein
-- neuer Provider. RLS/Grants wie 0181 + DEFINER-SELECT auf project_file.
-- ═══════════════════════════════════════════════════════════════════════
ALTER TABLE "project_file" ADD COLUMN "visible_to_customer" boolean DEFAULT false NOT NULL;
--> statement-breakpoint
DO $f1017_replace_resolver$
DECLARE
...
BEGIN
  ...
  SELECT COALESCE(
    ...
    FROM (
      SELECT CASE domain_event.event_type
               ...
               WHEN 'installation.handover_countersigned' THEN 'handover_countersigned'
             END AS type,
       ...
       WHERE domain_event.workspace_id = invite_row.workspace_id
         ...
         AND domain_event.event_type IN (
               ...
               'installation.handover_countersigned'
             )
         AND installation.project_id = invite_row.project_id
    ) AS timeline_row;
  ...
  -- F10-14: NULL-Label = reine Sichtbarkeits-Zeile (faellt nicht in
  -- statusLabels; uebersetzte Portal-Fallbacks je Sprache greifen).
  SELECT pg_catalog.jsonb_object_agg(label_row.source_key, label_row.label)
   ...
   WHERE label_row.workspace_id = invite_row.workspace_id
     ...
     AND label_row.label IS NOT NULL;
  ...
  -- F10-14: Sichtbarkeit je Anzeigestand (nur gesetzte Zeilen, nie
  -- interne Schluessel; ungemappt = leeres Objekt, alles sichtbar).
  SELECT pg_catalog.jsonb_object_agg(vis_row.source_key, vis_row.visible)
    INTO status_visibility_map
    FROM public.portal_status_label AS vis_row
   WHERE vis_row.workspace_id = invite_row.workspace_id
     AND vis_row.scope = 'installation';

  IF installation_entry IS NOT NULL THEN
    installation_entry := installation_entry || pg_catalog.jsonb_build_object(
      'statusVisibility', COALESCE(status_visibility_map, '{}'::jsonb)
    );
  END IF;
  ...
  SELECT COALESCE(pg_catalog.jsonb_agg(req), '[]'::jsonb)
    ...
    FROM (
      SELECT pg_catalog.jsonb_build_object(
        ...
        'fileType', freq.file_type,
        'subsidyLinked', freq.subsidy_case_id IS NOT NULL,
  ...
  -- F10-17: Kunden-Dateien des Invite-Projekts (nur freigeschaltete —
  -- nie Storage-Key/Pruefsumme; newest-first wie die interne Liste).
  SELECT COALESCE(pg_catalog.jsonb_agg(pf), '[]'::jsonb)
    INTO project_files_list
    FROM (
      SELECT pg_catalog.jsonb_build_object(
        'id', pfile.id,
        'originalFilename', pfile.original_filename,
        'contentType', pfile.content_type,
        'byteSize', pfile.byte_size,
        'createdAt', pfile.created_at
      ) AS pf
       ...
       ORDER BY pfile.created_at DESC, pfile.id DESC
    ) AS pfiles;
...
$f1017_resolve_token$;$ddl$;
  ...
  ELSE
    ...
    -- F10-17: Kunden-Datei-Projektion des Definers (Muster file_request).
    GRANT SELECT ON public.project_file TO app_owner;
    SET ROLE app_owner;
    EXECUTE $ddl$CREATE OR REPLACE FUNCTION public.resolve_portal_public_view(requested_token_hash bytea)
RETURNS jsonb
LANGUAGE plpgsql
...
$f1017_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1017_replace_resolver$;--> statement-breakpoint
CREATE FUNCTION public.read_portal_project_file_artifact(
...
SECURITY DEFINER
SET search_path = pg_catalog
AS $f1017_read_portal_project_file_artifact$
DECLARE
  located_workspace_id uuid;
  located_invite_id uuid;
  invite_row public.portal_invite%ROWTYPE;
...
BEGIN
  ...
  SELECT pfile.original_filename,
   ...
   WHERE pfile.workspace_id = invite_row.workspace_id
     ...
     AND pfile.visible_to_customer = true;
END;
$f1017_read_portal_project_file_artifact$;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.read_portal_project_file_artifact(bytea, uuid) FROM PUBLIC;--> statement-breakpoint
-- Owner-Tanz NUR im Testmodus (Muster 0064/0104/0116 — dort identischer Befund:
...
-- als app_owner).
DO $f1017_read_artifact_owner_dance$
DECLARE
  v_app name := current_user;
BEGIN
  IF v_app <> 'app_owner' THEN
    ...
    GRANT CREATE ON SCHEMA public TO app_owner;
    ALTER FUNCTION public.read_portal_project_file_artifact(
      bytea, uuid
    ) OWNER TO app_owner;
    REVOKE CREATE ON SCHEMA public FROM app_owner;
    SET ROLE app_owner;
    REVOKE ALL ON FUNCTION public.read_portal_project_file_artifact(
      bytea, uuid
    ) FROM PUBLIC;
    EXECUTE pg_catalog.format(
      'GRANT EXECUTE ON FUNCTION public.read_portal_project_file_artifact(
        bytea, uuid
      ) TO %I',
      v_app
    );
  ...
  END IF;
END
$f1017_read_artifact_owner_dance$;--> statement-breakpoint

DO $f1017_read_artifact_acl$
BEGIN
