-- F10-03 Installation-Tab — resolve_portal_public_view projiziert
-- zusaetzlich den Installationsstand (status/completedAt/handoverAt,
-- keine Namen/Notizen, Privacy-Spec). NULL ohne Installation.
...
$f1001_resolve_token$;$ddl$;
  ...
  END IF;
END
$f1003_replace_resolver$;--> statement-breakpoint

DO $f1003_grants$
BEGIN
  IF current_user <> 'app_owner' THEN
    -- Projektion-Relationen des Definers (Strict: Tabellen-Ownership).
    GRANT SELECT ON public.installation TO app_owner;
  ...
  END IF;
END
$f1003_grants$;--> statement-breakpoint
