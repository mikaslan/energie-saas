...
-- Validator erlaubt `kind = 'text'` je Punkt (abhakbar wie Aufgabe) und
-- den optionalen Key `value` (Antworttext, max. 2000 Zeichen) nur dort
-- (Spiegel-Regel zu description, F7-02C-Präzedenz). Keine neue Tabelle,
-- keine neue Permission (checklist.write), keine RLS-Aenderung.
      ...
      LOOP
        ...
        IF pg_catalog.jsonb_typeof(item_record.value) <> 'object'
           ...
           OR (item_record.value ? 'kind'
               ...
               AND NOT (item_record.value->>'kind' IN ('task','title','description','radio','text')))
