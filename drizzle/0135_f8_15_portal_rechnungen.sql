-- ═══════════════════════════════════════════════════════════════════════
-- F8-15 Portal-Rechnungssicht (Katalog F8.5 „Portal-Sicht"): Der
-- Portal-DEFINER projiziert je Invite-Projekt zusaetzlich `invoices` —
-- nur ausgestellte Geldbelege (invoice/credit_note, status issued) mit
-- Nummer/Art/Ausstellung/Brutto/Zahlstand. Keine Positionen, keine
-- Skonto-Konditionen, keine Snapshots, keine Entwuerfe/stornierten.
-- Rechte: GRANT SELECT ON commercial_document an app_owner (Muster
-- file_request); RLS ungeaendert, keine neue Permission.
-- ═══════════════════════════════════════════════════════════════════════
DO $f8015_replace_resolver$
DECLARE
...
$f8015_resolve_token$;$ddl$;
  ...
  END IF;
END
$f8015_replace_resolver$;--> statement-breakpoint
DO $f8015_grants$
BEGIN
