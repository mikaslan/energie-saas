-- F7-02J: Komponentenlisten-Punkt (Katalog F7.2: component-list als
-- Anzeige-Punkt wie title/description — weder Pflicht noch abhakbar, keine
-- Nutzlast). Vollkopie von 0175 plus kind='component-list' (kind-IN und
-- Anzeige-Regel). Der Tree speichert nur die Art (kein Inhalt, kein
-- Snapshot — 03B „Art nie Inhalt"); die Anzeige projiziert die
-- Workbook-Stueckliste read-only (F7-08, installation.read-gated).
      ...
      LOOP
        ...
        IF pg_catalog.jsonb_typeof(item_record.value) <> 'object'
           ...
           OR (item_record.value ? 'kind'
               ...
               AND NOT (item_record.value->>'kind' IN ('task','title','description','radio','text','multi','image','signature','component-list')))
           ...
           OR ((item_record.value->>'kind' IN ('title','description','component-list'))
