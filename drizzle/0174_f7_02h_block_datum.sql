-- F7-02H: Block-Faelligkeitsdatum (Katalog F7.2: „Block (zuweisbar,
-- Faelligkeitsdatum)"). Vollkopie von 0173 plus optionaler Block-Key
-- `dueDate` (Kalendertag YYYY-MM-DD, fehlend/null = kein Datum).
-- Das Datum ist STRUKTUR (kein Strip in `_f704_checklist_structure`):
-- Editor-Antwort-Saves mit Datumswechsel verweigert die Kapsel als
-- 42501-Strukturwechsel (F7-03b-Spiegel). Exaktheit per Regex plus
-- to_date/to_char-Roundtrip (weist 2026-02-31 ohne Exception-Pfad ab).
-- Keine neue Tabelle, keine neue Permission (checklist.write), keine
-- RLS-Aenderung.
    ...
    LOOP
      ...
      LOOP
        ...
        IF pg_catalog.jsonb_typeof(item_record.value) <> 'object'
           ...
           OR item_record.value - ARRAY['id','title','done','required','visible','irrelevant','visibleIf','kind','description','value','photo','componentId']::text[]
              <> '{}'::jsonb
           ...
           OR (item_record.value ? 'kind'
               ...
               AND NOT (item_record.value->>'kind' IN ('task','title','description','radio','text','multi','image')))
