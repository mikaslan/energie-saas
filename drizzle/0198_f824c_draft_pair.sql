-- ═══════════════════════════════════════════════════════════════════════
-- F8-24c: Draft-Vorschau als drittes Render-Paar auf
-- commercial_document_render_job (template/recipe/pair-CHECKs erweitert,
-- Kreuzmix per pair_ck ausgeschlossen). Additiv, kein Dispatch-Gate
-- (Vorschau ist on-demand, kein Versand-Artefakt).
-- ═══════════════════════════════════════════════════════════════════════
...
ALTER TABLE "commercial_document_render_job" DROP CONSTRAINT "commercial_document_render_job_pair_ck";--> statement-breakpoint
ALTER TABLE "commercial_document_render_job" ADD CONSTRAINT "commercial_document_render_job_template_ck" CHECK ("commercial_document_render_job"."template_version" in ('invoice-pdf-template.v1', 'invoice-payment-template.v1', 'draft-pdf-template.v1'));--> statement-breakpoint
ALTER TABLE "commercial_document_render_job" ADD CONSTRAINT "commercial_document_render_job_recipe_ck" CHECK ("commercial_document_render_job"."renderer_recipe" in ('invoice-pdf-renderer-recipe.v1', 'invoice-payment-renderer-recipe.v1', 'draft-pdf-renderer-recipe.v1'));--> statement-breakpoint
ALTER TABLE "commercial_document_render_job" ADD CONSTRAINT "commercial_document_render_job_pair_ck" CHECK ((("commercial_document_render_job"."template_version" = 'invoice-pdf-template.v1') and ("commercial_document_render_job"."renderer_recipe" = 'invoice-pdf-renderer-recipe.v1')) or (("commercial_document_render_job"."template_version" = 'invoice-payment-template.v1') and ("commercial_document_render_job"."renderer_recipe" = 'invoice-payment-renderer-recipe.v1')) or (("commercial_document_render_job"."template_version" = 'draft-pdf-template.v1') and ("commercial_document_render_job"."renderer_recipe" = 'draft-pdf-renderer-recipe.v1')));--> statement-breakpoint

-- F8-24a Overdue-Sweep-Grants leben NICHT hier, sondern im
-- Rollenvertrag (M3-02c-Praezedenz: applyRoleContract revoked
-- Tabellen-Grants aus Migrationen ohne Re-Grant wieder; Verify
-- pinnt sie exakt). Siehe scripts/db-role-contract.mts (F8-24a-Block,
-- Sonde: overdue_sweep_workspace aus 0200).

-- ═══════════════════════════════════════════════════════════════════════
-- F8-24c: pgboss-Dispatch draft-pdf.render (Spiegel M3-02c 0193, eigene
-- Queue). ID-only-Payload, idempotenter Dispatch-Key, Schema-Gate.
-- ═══════════════════════════════════════════════════════════════════════
DO $f824c_draft_dispatch_migration$
DECLARE
  pgboss_owner text;
BEGIN
  ...
  IF pgboss_owner IS NULL THEN
    IF CURRENT_USER = SESSION_USER
      ...
      RETURN;
    END IF;
    RAISE EXCEPTION 'F8-24c draft dispatch: pgboss-Schema fehlt';
  END IF;
  IF pgboss_owner <> 'app_worker' THEN
    RAISE EXCEPTION 'F8-24c draft dispatch: pgboss muss app_worker gehoeren';
  END IF;
  IF NOT pg_catalog.pg_has_role(SESSION_USER, 'app_worker', 'SET') THEN
    RAISE EXCEPTION 'F8-24c draft dispatch: app_migrator braucht SET auf app_worker';
  END IF;

  EXECUTE 'SET LOCAL ROLE app_worker';
  EXECUTE $dispatch_ddl$
    CREATE OR REPLACE FUNCTION pgboss.enqueue_draft_pdf_render(
      workspace_id uuid,
      job_id uuid
    )
    RETURNS void
    ...
    BEGIN
      IF NULLIF(
        ...
        RAISE EXCEPTION 'draft PDF dispatch: workspace context mismatch'
          USING ERRCODE = '42501';
      END IF;
      ...
      SELECT domain_job.status,
       ...
       WHERE domain_job.workspace_id = $1
         ...
         AND domain_job.template_version = 'draft-pdf-template.v1'
         AND domain_job.renderer_recipe = 'draft-pdf-renderer-recipe.v1'
         AND domain_job.input_json ->> 'schemaVersion' = 'draft-pdf-input.v1'
       ...
       FOR SHARE;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'draft PDF dispatch: keine zustellbare Reservation'
          USING ERRCODE = '42501';
      END IF;
      ...
      IF dispatch_start_after IS NULL THEN
        RAISE EXCEPTION 'draft PDF dispatch: Zustellzeit fehlt';
      END IF;

      PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtextextended($2::text, 2408240311)
      );
      SELECT pg_catalog.max(version)
        INTO runtime_pgboss_version
        FROM pgboss.version;
      IF runtime_pgboss_version IS DISTINCT FROM 38 THEN
        RAISE EXCEPTION 'draft PDF dispatch: pg-boss-Schemaversion driftet';
      END IF;
      SELECT *
        INTO queue_config
        FROM pgboss.queue AS queue
       WHERE queue.name = 'draft-pdf.render';
      IF NOT FOUND
        ...
        RAISE EXCEPTION 'draft PDF dispatch: Queuevertrag fehlt oder driftet';
      END IF;
      ...
      IF EXISTS (
        SELECT 1
         ...
         WHERE queued_job.name = 'draft-pdf.render'
      ...
      IF EXISTS (
        ...
        RAISE EXCEPTION 'draft PDF dispatch: aktiver Job verletzt Vertrag';
      END IF;
      ...
      IF NOT FOUND THEN
        IF EXISTS (
          ...
          RETURN;
        END IF;
        RAISE EXCEPTION 'draft PDF dispatch: unerwarteter pg-boss-Konflikt';
      END IF;
    END
    $dispatch_body$
  $dispatch_ddl$;

  EXECUTE 'GRANT EXECUTE ON FUNCTION pgboss.enqueue_draft_pdf_render(uuid, uuid) TO app_runtime';
  EXECUTE 'GRANT EXECUTE ON FUNCTION pgboss.enqueue_draft_pdf_render(uuid, uuid) TO app_worker';
  EXECUTE 'SET LOCAL ROLE app_owner';
END
$f824c_draft_dispatch_migration$;