-- F7-02I: Unterschrift-Punkt (Katalog F7.2: signature mit Typ
-- Kunde/Techniker/Dritter). Vollkopie von 0174 plus kind='signature'
-- (Arbeitspunkt wie image: required/done erlaubt) und optionalem Key
-- `signerRole` nur dort (STRUKTUR, kein Strip — Setzen nur mit
-- Strukturrecht, dueDate-Spiegel F7-02h). Der wiederverwendete `photo`-
-- Key haelt am Signaturpunkt das Signatur-PNG (Spiegel auf image und
-- signature erweitert; Bytes bleiben Nutzlast, Editor darf signieren).
      ...
      LOOP
        ...
        IF pg_catalog.jsonb_typeof(item_record.value) <> 'object'
           ...
           OR (item_record.value ? 'kind'
               ...
               AND NOT (item_record.value->>'kind' IN ('task','title','description','radio','text','multi','image','signature')))
