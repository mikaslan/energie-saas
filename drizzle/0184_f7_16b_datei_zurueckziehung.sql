-- ═══════════════════════════════════════════════════════════════════════
-- F7-16b Datei-Zurückziehung (Katalog F7.1): withdrawn-Flag je
-- Projekt-Datei (DEFAULT false = aktiv; one-way, terminal) + Resolver-
-- WHERE (0182-Vollkopie, beide Rümpfe) + Kapsel-BEIDE-WHEREs
-- (0183-Vollkopie INKL. Download-Insert, je Rumpf SELECT INTO UND
-- RETURN QUERY — Sync-Pflicht, sonst Log-ohne-Lieferung). Portal-
-- Sichtbarkeit = sichtbar UND NICHT zurückgezogen; intern bleibt die
-- Zeile mit Badge sichtbar, Download weiter erlaubt (Beleg). Kein
-- physisches DELETE (WORM), keine neue Permission, keine neuen Grants
-- (UPDATE-Grant table-level, SELECT-Grants bestehen).
-- ═══════════════════════════════════════════════════════════════════════
ALTER TABLE "project_file" ADD COLUMN "withdrawn" boolean DEFAULT false NOT NULL;
--> statement-breakpoint
-- F7-16b Hand-Anteil 1: Resolver-Replace (0182-Vollkopie — 0183 fasste
-- den Resolver NICHT an; Owner-Tanz, beide Rümpfe, Signatur unverändert).
DO $f716b_replace_resolver$
DECLARE
...
$f716b_resolve_token$;$ddl$;
  ...
  END IF;
END
$f716b_replace_resolver$;--> statement-breakpoint
-- F7-16b Hand-Anteil 2: Kapsel-Replace (0183-Vollkopie INKL.
-- Download-Insert, beide Rümpfe, Signatur unverändert; Withdraw in
-- BEIDEN WHEREs je Rumpf — sonst Log-ohne-Lieferung).
DO $f716b_replace_artifact$
DECLARE
  v_app name := current_user;
BEGIN
  IF v_app = 'app_owner' THEN
...
SECURITY DEFINER
SET search_path = pg_catalog
AS $f716b_read_portal_project_file_artifact$
DECLARE
  located_workspace_id uuid;
  located_invite_id uuid;
  invite_row public.portal_invite%ROWTYPE;
...
BEGIN
  ...
  SELECT pfile.original_filename,
   ...
   WHERE pfile.workspace_id = invite_row.workspace_id
     ...
     AND pfile.withdrawn = false;
END;
$f716b_read_portal_project_file_artifact$;$ddl$;
  ELSE
    EXECUTE pg_catalog.format(
      'GRANT app_owner TO %I WITH inherit false, set true', v_app
    );
