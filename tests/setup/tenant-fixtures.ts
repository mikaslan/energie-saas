...
export const tenantFixtures: Record<string, (tx: TenantTx, wsId: string) => Promise<void>> = {
  ...
  // F16-08 (0136): Planungs-Vorlagen — nur workspace-FK, RLS
  // tenant_isolation, keine Actor-Policies.
  planning_template: async (tx, wsId) => {
    await tx.execute(sql`
      insert into planning_template (
        id, workspace_id, name, name_normalized, mode,
        active, position, created_by
      ) values (
        ${randomUUID()}::uuid, ${wsId}::uuid, 'Fixture Planung',
        'fixture planung', 'quick',
        true, 0,
        ${randomUUID()}::uuid
      )
    `);
  ...
  // F3-04a (0273): H-Rastergruppe zum frisch angelegten Dach.
  planning_panel_group: async (tx, wsId) => {
    const { projectId, siteId } = await fixtureProjectGraph(tx, wsId);
    const { userId } = await fixtureMembership(tx, wsId, "editor");
    const createdSource = await tx.execute<{ id: string }>(sql`
      insert into planning_source (workspace_id, project_id, site_id, kind, created_by)
  ...
  // F3-05b (0275): String-Optimierer am Fixture-String.
  planning_string_equipment: async (tx, wsId) => {
    const { projectId, siteId } = await fixtureProjectGraph(tx, wsId);
    const { userId } = await fixtureMembership(tx, wsId, "editor");
    const createdInverter = await tx.execute<{ id: string }>(sql`
      insert into planning_inverter (
    ...
    const groupId = createdGroup.rows[0]?.id;
    if (!groupId) throw new Error("planning_panel_group-Fixture lieferte keine ID");
    const createdString = await tx.execute<{ id: string }>(sql`
      insert into planning_string (
        workspace_id, inverter_id, tracker_slot, label,
        member_json, created_by
      ) values (
    ...
    if (!stringId) throw new Error("planning_string-Fixture lieferte keine ID");
    await tx.execute(sql`
      insert into planning_string_equipment (
        workspace_id, string_id, scope, panel_ref_json, equipment, created_by
      ) values (
        ${wsId}::uuid, ${stringId}::uuid, 'string', null,
        'optimizer', ${userId}::uuid)
    `);
  },
  // F3-04b (0276): Deselect-Zelle (1,1) an der Fixture-Gruppe.
  planning_panel_deselect: async (tx, wsId) => {
    const { projectId, siteId } = await fixtureProjectGraph(tx, wsId);
    const { userId } = await fixtureMembership(tx, wsId, "editor");
    const createdSource = await tx.execute<{ id: string }>(sql`
      insert into planning_source (workspace_id, project_id, site_id, kind, created_by)
    ...
    await tx.execute(sql`
      insert into planning_panel_deselect (
        ...
        ${wsId}::uuid, ${groupId}::uuid, 1, 1, 'Fixture-Abwahl',
        ${userId}::uuid)
    `);
  },
  // F3-05c (0277): Member-Range (Hälfte) am Fixture-String.
  planning_string_member: async (tx, wsId) => {
    const { projectId, siteId } = await fixtureProjectGraph(tx, wsId);
    const { userId } = await fixtureMembership(tx, wsId, "editor");
    const createdInverter = await tx.execute<{ id: string }>(sql`
      insert into planning_inverter (
    ...
    await tx.execute(sql`
      insert into planning_string_member (
        ...
        ${wsId}::uuid, ${stringId}::uuid, ${groupId}::uuid,
        1, 2, 1, 6, ${userId}::uuid)
    `);
  },
};
