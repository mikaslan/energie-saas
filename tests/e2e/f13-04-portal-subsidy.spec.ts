...
/**
 * F13-04 Förderstand im Portal — Chromium-E2E (isolierter Workspace).
 * Akte: Förderakte anlegen → Programm setzen → BzA einreichen →
 * Portal-Link per UI → Übersicht zeigt Förderstand (ohne BzA-Nummer).
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "editorEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F13-04-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F13-04-E2E-01: Portal-Übersicht zeigt Förderstand ohne BzA-Nummer", async ({ page }) => {
  ...
  await form.getByLabel("Name *").fill("E2E Foerderportal");
  await form.getByLabel("Telefon").fill("0151 45678901");
  ...
  await page.getByTestId("subsidy-case-bza-number").fill("BZA-E2E-INTERN");
  ...
  await expect(page.locator("main")).not.toContainText("BZA-E2E-INTERN");

  expect(errors, "Browser-Konsole und Page-Errors der Förderportal-Grenze").toEqual([]);
});
