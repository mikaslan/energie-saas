...
/**
 * F8-05 Teilrechnung zum Auftrag — Chromium-E2E.
 * Auftragsbestätigung (Entwurf, 2 Positionen, netto 9.500 €) per SQL
 * seeden → Detail-Panel „Teilrechnungen“, Modus Prozent (30 %) →
 * Teilrechnungs-Entwurf mit Sammellinie, Kette + Restbetrag sichtbar.
 */
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F8-05-E2E-State ist unvollständig.");
  }
  ...
  try {
    ...
    await client.query(
      `insert into workspace_invoicing_settings (
         ...
         $1::uuid, 'F805 GmbH', 'office@f805.example',
         'Strasse 1', '10115', 'Berlin', 'DE',
         'F805 GmbH', 'DE89370400440532013000', 'MARKDEF1100',
...
test("F8-05-E2E-01: Prozent-Teilrechnung → Kette mit Restbetrag", async ({ page }) => {
  ...
  // Sammellinie 30 % von 9.500 € netto = 2.850 € netto, 3.391,50 € brutto.
  const lines = page.locator('[data-invoice-detail="lines"]');
  await expect(lines.getByText(/Teilrechnung-Anteil 1 /u)).toBeVisible();
  await expect(lines.getByText("3.391,50 €")).toBeVisible();

  // Zurück zur AB: Kette mit Folge 1 + Rest 7.913,50 € sichtbar.
  await page.goto(orderConfirmationsPath());
