...
/**
 * F7-02I Unterschrift-Punkt (Katalog F7.2) — Chromium-E2E (isolierter
 * Workspace). Admin setzt die Rolle, zeichnet eine Unterschrift (Canvas),
 * speichert (Reload-fest); leeres Speichern warnt; Viewer sieht lesend.
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "adminEmail", "viewerEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F7-02I-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F7-02I-E2E-01: Unterschrift-Punkt ist persistent und lesbar", async ({ page }) => {
  ...
  await form.getByLabel("Name *").fill("E2E Unterschrift");
  ...
  await page.getByLabel("Punkt-Name 1.1").fill(signTitle);
  const signItem = page.locator("li").filter({ has: page.getByLabel("Punkt-Name 1.1") });
  await signItem.getByLabel("Typ").selectOption("signature");
  await signItem.getByLabel(`${signTitle}: Rollentyp`).selectOption("kunde");

  // Leeres Speichern warnt (kein Upload, kein Fehler-Alert).
  await signItem.getByRole("button", { name: "Unterschrift speichern" }).click();
  await expect(signItem.getByText("Bitte zuerst unterschreiben.", { exact: true })).toBeVisible();

  // Canvas ist kein labelierbares Element: direkter Locator (aria-label
  // bleibt für AT/Axe an der Zeichenfläche).
  const canvas = signItem.locator("canvas");
  const box = await canvas.boundingBox();
  if (!box) throw new Error("Signatur-Canvas unsichtbar.");
  ...
  await page.mouse.up();
  await signItem.getByRole("button", { name: "Unterschrift speichern" }).click();
  const preview = signItem.getByRole("img", { name: `${signTitle}: Unterschrift-Vorschau` });
  ...
  await page.reload();
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  const signReloaded = page.locator("li").filter({ has: page.getByLabel("Punkt-Name 1.1") });
  await expect(signReloaded.getByLabel(`${signTitle}: Rollentyp`)).toHaveValue("kunde");
  const previewReloaded = signReloaded.getByRole("img", { name: `${signTitle}: Unterschrift-Vorschau` });
  await expect(previewReloaded).toBeVisible();
  await expect(previewReloaded).toHaveAttribute("src", firstSrc!);

  // Viewer (lesend): Vorschau sichtbar, keine Zeichenfläche.
  await page.context().clearCookies();
  await page.goto(url);
  await loginWithRealOtp(page, data.viewerEmail, url);
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  const viewerSign = page.locator("li", { hasText: signTitle });
  await expect(viewerSign.getByRole("img", { name: `${signTitle}: Unterschrift-Vorschau` })).toBeVisible();
  await expect(viewerSign.locator("canvas")).toHaveCount(0);
  await expect(viewerSign.getByLabel("Typ")).toHaveCount(0);

  await expectNoWcagAaAxeViolations(page, "F7-02I-Signatur");

  expect(errors, "Browser-Konsole und Page-Errors des Unterschrift-Punkts").toEqual([]);
});
