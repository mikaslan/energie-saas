...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F8-08-E2E-State ist unvollständig.");
  }
  ...
  try {
    ...
    await client.query(
      `insert into workspace_invoicing_settings (
         ...
         $1::uuid, 'F808 GmbH', 'office@f808.example',
         'Strasse 1', '10115', 'Berlin', 'DE',
         'F808 GmbH', 'DE89370400440532013000', 'MARKDEF1100',
...
test("F8-08-E2E-01: Prozent-Teilrechnung → Rest schließen → Kette geschlossen", async ({ page }) => {
  ...
  // Kette schließen: Rest-Modus ist jetzt aktiv (vorher ohne Kette
  // deaktiviert — hier implizit belegt, da die Kette eine Zeile hat).
  await panel.getByRole("radio", { name: /Restbetrag/ }).check();
  await panel.getByTestId("partial-invoice-submit").click();
  ...
  // Rest (70 % von 9.500 € netto) = 6.650 € netto, 7.913,50 € brutto.
  const lines = page.locator('[data-invoice-detail="lines"]');
  await expect(lines.getByText(/Restbetrag zu /u)).toBeVisible();
  await expect(lines.getByText("7.913,50 €")).toBeVisible();

  // Zurück zur AB: Kette [Prozent, Rest] + Rest 0,00 € sichtbar.
  await page.goto(orderConfirmationsPath());
  ...
  await expect(chainList.getByText("Rest").first()).toBeVisible();
