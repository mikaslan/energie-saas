...
/**
 * F8-07 Zahlungsplan-Modus (Scheme 30/40/30) — Chromium-E2E.
 * Auftragsbestätigung (Entwurf, 2 Positionen, netto 9.500 €) per SQL
 * seeden → Detail-Panel „Teilrechnungen“, Modus Zahlungsplan →
 * Tranche 1 als Sammellinie (30 %), Kette + Restbetrag sichtbar.
 */
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F8-07-E2E-State ist unvollständig.");
  }
  ...
  try {
    ...
    await client.query(
      `insert into workspace_invoicing_settings (
         ...
         $1::uuid, 'F807 GmbH', 'office@f807.example',
         'Strasse 1', '10115', 'Berlin', 'DE',
         'F807 GmbH', 'DE89370400440532013000', 'MARKDEF1100',
...
test("F8-07-E2E-01: Zahlungsplan-Tranche → Kette mit Restbetrag", async ({ page }) => {
  ...
  await panel.getByRole("radio", { name: /Zahlungsplan/ }).check();
  ...
  // Tranche 1 (30 % von 9.500 € netto) = 2.850 € netto, 3.391,50 € brutto.
  const lines = page.locator('[data-invoice-detail="lines"]');
  await expect(lines.getByText(/Zahlungsplan-Tranche 1 /u)).toBeVisible();
  await expect(lines.getByText("3.391,50 €")).toBeVisible();

  // Zurück zur AB: Kette mit „Zahlungsplan 30 %" + Rest 7.913,50 € sichtbar.
  await page.goto(orderConfirmationsPath());
  ...
  await expect(summary).toContainText("1 Teilrechnung gestellt.");
  await expect(summary).toContainText("7.913,50 €");
  await expect(page.getByText("Zahlungsplan 30 %").first()).toBeVisible();
