...
/**
 * F8-20 Monats-ZIP — Chromium-E2E.
 * Rechnungs-Entwurf (2 Positionen, 19 %) per SQL seeden → über die
 * Listenaktion „Ausstellen" → Berichte-Seite → Monats-ZIP →
 * Download beginnt mit `PK`, `summary.csv` enthält die Belegnummer;
 * Axe sauber; keine Konsolenfehler.
 *
 * Hinweis: Ohne Worker-Lauf existiert kein versiegeltes PDF-Artefakt —
 * der ZIP bleibt ehrlich partiell (Zeile mit leeren pdf_*-Spalten),
 * die Buchungsdaten sind trotzdem enthalten.
 */
...
const INVOICE_NAME = "F820-Rechnung";
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F8-20-E2E-State ist unvollständig.");
  }
  ...
  try {
    ...
    await client.query(
      `insert into workspace_invoicing_settings (
         ...
         $1::uuid, 'F820 GmbH', 'office@f820.example', 'DE123456789',
         'Strasse 1', '10115', 'Berlin', 'DE',
         'F820 GmbH', 'DE89370400440532013000', 'MARKDEF1100',
    ...
    await client.query(
      `insert into contact (
         ...
         $1::uuid, 'F820 Kundin', 'Fixture', 'Contact',
         'kundin@f820.example', 'kundin@f820.example',
    ...
    const inserted = await client.query(
      `insert into commercial_document (
         ...
         (select id from contact
           where workspace_id = $1::uuid and email_normalized = 'kundin@f820.example'
           limit 1),
...
test("F8-20-E2E-01: Berichte → Monats-ZIP mit Belegnummer in summary.csv", async ({ page }) => {
  ...
  // Echter Pfad: Entwurf ausstellen, damit er in den Monats-ZIP fällt.
  ...
  const zipLink = page.getByTestId("monats-zip-download");
  ...
  expect(suggested).toMatch(/^monatsunterlagen-20\d{2}-\d{2}\.zip$/u);

  const entries = unzipSync(bytes);
  const names = Object.keys(entries).filter((name) => !name.endsWith("/"));
  expect(names).toContain("summary.csv");
  const summary = Buffer.from(entries["summary.csv"] as Uint8Array).toString("utf8");
  expect(summary.split("\r\n")[0]).toBe(
    "typ;nummer;ausstellungsdatum;kontakt;netto;steuer;brutto;pdf_datei;pdf_sha256",
  );
  // Unser ausgestellter Beleg steht in der Summary (Kontaktname statt
  // Nummernformat: die Serie ist workspace-konfigurierbar). Ohne
  // Worker-Artefakt bleibt die Zeile ehrlich partiell (leere pdf_*-Spalten).
  expect(summary).toContain("F820 Kundin");
