...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F8-12-E2E-State ist unvollständig.");
  }
  ...
  try {
    ...
    await client.query(
      `insert into workspace_invoicing_settings (
         ...
         $1::uuid, 'F812 GmbH', 'office@f812.example',
         'Strasse 1', '10115', 'Berlin', 'DE',
         'F812 GmbH', 'DE89370400440532013000', 'MARKDEF1100',
...
test("F812-E2E-01: Prozent-Teilrechnung → Teil-Rest → Kette offen mit geschrumpftem Rest", async ({ page }) => {
  ...
  const success = page.getByTestId("partial-invoice-success");
  await expect(success).toContainText("Teilrechnungs-Entwurf angelegt");
  const firstHref = await success.getByRole("link", { name: "Rechnung öffnen" }).getAttribute("href");

  // Teil-Rest: Modus ist jetzt aktiv (vorher ohne Kette deaktiviert —
  // hier implizit belegt, da die Kette eine Zeile hat). 50 % vom Rest
  // (70 % von 9.500 € netto) = 3.325 € netto, 3.956,75 € brutto.
  await panel.getByRole("radio", { name: /Teil-Rest/ }).check();
  await panel.getByTestId("partial-percent").fill("50");
  await panel.getByTestId("partial-invoice-submit").click();

  // Auf die ZWEITE Anlage warten (Kette zählt hoch), sonst griffe der
  // Link noch auf die erste Teilrechnung (stale success).
  await expect(page.getByTestId("partial-chain-summary")).toContainText("2 Teilrechnungen gestellt.");
  const secondHref = await success.getByRole("link", { name: "Rechnung öffnen" }).getAttribute("href");
  ...
  await expect(lines.getByText(/Teil-Restbetrag zu /u)).toBeVisible();
  await expect(lines.getByText("3.956,75 €")).toBeVisible();

  // Zurück zur AB: Kette [Prozent, Teil-Rest] + geschrumpfter Rest
  // (3.956,75 €) sichtbar — Kette bleibt offen.
  await page.goto(orderConfirmationsPath());
  await page.getByRole("row").filter({ hasText: AB_NAME }).getByRole("link", { name: AB_NAME }).click();
  const summary = page.getByTestId("partial-chain-summary");
  await expect(summary).toContainText("2 Teilrechnungen gestellt.");
  await expect(summary).toContainText("3.956,75 €");
  const chainList = page.getByRole("list", { name: "Gestellte Teilrechnungen" });
  await expect(chainList.getByText("Teil-Rest 50 %").first()).toBeVisible();
