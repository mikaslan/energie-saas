...
/**
 * F7-02E Freitext-Antwort (Katalog F7.2, Slice B ohne Diktat) — Chromium-E2E
 * (isolierter Workspace). Admin legt zwei Textpunkte an, füllt den ersten
 * (Pflicht-Gate zählt offen), hakt ihn ab (Antworttext bleibt Nutzlast).
 * Speichern + Reload belegt die Persistenz; Viewer sieht den Wert lesend.
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "adminEmail", "viewerEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F7-02E-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F7-02E-E2E-01: Freitext-Antwort ist persistent und gate-neutral", async ({ page }) => {
  ...
  await form.getByLabel("Name *").fill("E2E Freitext-Antwort");
  ...
  const firstTitle = `Zählerstand ${stamp}`;
  const secondTitle = `Dachfläche ${stamp}`;
  const answerText = "42.195 kWh";
  ...
  await firstItem.getByLabel("Typ").selectOption("text");
  await firstItem.getByLabel(`${firstTitle}: Antworttext`).fill(answerText);
  ...
  await secondItem.getByLabel("Typ").selectOption("text");

  // Pflicht-Gate zählt den offenen Textpunkt wie eine Aufgabe.
  await expect(page.getByText("Noch 1 Pflichtpunkt offen.", { exact: true })).toBeVisible();

  const firstCheckbox = firstItem.getByRole("checkbox", { name: firstTitle, exact: true });
  await expect(firstCheckbox).toBeVisible();
  await firstCheckbox.check();
  await expect(firstCheckbox).toBeChecked();
  // Antworttext bleibt Nutzlast (kein Auto-Check nötig), Pflicht erfüllt.
  await expect(firstItem.getByLabel(`${firstTitle}: Antworttext`)).toHaveValue(answerText);
  ...
  await expect(firstReloaded.getByLabel(`${firstTitle}: Antworttext`)).toHaveValue(answerText);
  await expect(page.getByText("Punkte: 1/2", { exact: true })).toBeVisible();

  // Viewer (lesend): Antworttext sichtbar, ohne Editor.
  await page.context().clearCookies();
  await page.goto(url);
  await loginWithRealOtp(page, data.viewerEmail, url);
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  const viewerFirst = page.locator("li", { hasText: firstTitle });
  await expect(viewerFirst.getByText(answerText, { exact: true })).toBeVisible();
  ...
  await expectNoWcagAaAxeViolations(page, "F7-02E-Text");

  expect(errors, "Browser-Konsole und Page-Errors der Freitext-Antwort").toEqual([]);
});
