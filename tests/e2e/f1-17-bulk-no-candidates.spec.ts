...
/**
 * F1-17 Stub-ohne-Kandidaten — Chromium-E2E (isolierter Workspace).
 * Die Adresse ist dem lokalen Geoapify-Vertragsstub unbekannt (leere
 * Trefferliste): Der Import legt die Anfrage trotzdem an (created+legacy),
 * meldet „1 nicht geocodiert“ und löst keine Detailauflösung aus.
 */

// Muss der Stub-Anfrage F1_17_NO_CANDIDATE_QUERY in run.mts entsprechen:
// „Straße Hausnummer, PLZ Ort“.
const NO_CANDIDATE_CELLS = {
  street: "Nirgendweg",
  houseNumber: "999",
  postalCode: "00000",
  city: "Nirgendstadt",
...
function noCandidateXlsx(): Buffer {
  ...
  XLSX.utils.book_append_sheet(
    ...
    XLSX.utils.aoa_to_sheet([
      ...
      [
        "Bulk XLSX Ohne",
        "ohne@f117.test",
        NO_CANDIDATE_CELLS.street,
        NO_CANDIDATE_CELLS.houseNumber,
        NO_CANDIDATE_CELLS.postalCode,
        NO_CANDIDATE_CELLS.city,
      ],
    ]),
    "Import",
  );
...
test("F1-17-E2E-02: XLSX-Import ohne Stub-Kandidaten bleibt created+legacy", async ({ page }) => {
  ...
  await expect(report).toContainText("1 nicht geocodiert");
  await expect(page.getByRole("heading", { name: "Bulk XLSX Ohne" })).toBeVisible();
