...
/**
 * F8-22 DATEV-Sonderfaelle — Chromium-E2E (NICHT in Track-E-Vergabe lauffaehig:
 * braucht Migration 0197 tax_treatment; Owner laeuft spaeter).
 * 0-%-Rechnung (§12 Abs. 3) per SQL seeden → ueber die Listenaktion
 * „Ausstellen" → Berichte-Seite → DATEV-Stapel (SKR03) → Download enthaelt
 * `EXTF` + 0-%-Belegnummer + ESTIMATE-BU/Konto; Axe sauber; keine
 * Konsolenfehler.
 */
...
const INVOICE_NAME = "F822-Rechnung";
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F8-22-E2E-State ist unvollständig.");
  }
...
async function seedZeroRateDraftInvoice(): Promise<void> {
  const data = state();
  const pool = createDrainTrackedPool({ connectionString: data.databaseUrl, max: 1 });
  const client = await pool.connect();
  try {
    ...
    await client.query(
      `insert into workspace_invoicing_settings (
         ...
         $1::uuid, 'F822 GmbH', 'office@f822.example', 'DE123456789',
         'Strasse 1', '10115', 'Berlin', 'DE',
         'F822 GmbH', 'DE89370400440532013000', 'MARKDEF1100',
    ...
    await client.query(
      `insert into contact (
         ...
         $1::uuid, 'F822 Kundin', 'Fixture', 'Contact',
         'kundin@f822.example', 'kundin@f822.example',
    ...
    const inserted = await client.query(
      `insert into commercial_document (
         ...
         (select id from contact
           where workspace_id = $1::uuid and email_normalized = 'kundin@f822.example'
           limit 1),
         'EUR', 500000, 0, 500000,
...
test("F8-22-E2E-01: Berichte → DATEV-Stapel mit 0-%-Belegnummer", async ({ page }) => {
  test.setTimeout(180_000);
  const data = state();
  const errors = trackBrowserErrors(page);

  await grantInvoicingCapability();
  await seedZeroRateDraftInvoice();
  ...
  // 0-%-Zeile (§12 Abs. 3): ESTIMATE-Konto/BU statt Automatikkonto,
  // mit echter 0-%-Belegnummer in derselben Zeile.
  expect(content).toContain(";1400;8340;43;");
  const zeroRow = content.split("\r\n").find((line) => line.includes(";1400;8340;43;"));
  expect(zeroRow).toBeTruthy();
  expect(zeroRow).toMatch(/RE-\d{4}-\d{6}/u);
