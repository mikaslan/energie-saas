  ...
  try {
    ...
    await client.query(
      `insert into workspace_invoicing_settings (
         ...
         $1::uuid, 'F813 GmbH', 'office@f813.example',
         'Strasse 1', '10115', 'Berlin', 'DE',
         'F813 GmbH', 'DE89370400440532013000', 'MARKDEF1100',
...
test("F813-E2E-01: Prozent-Teilrechnung → Betrag → Kette offen mit geschrumpftem Rest", async ({ page }) => {
  ...
  // Betrag: 1.000 € netto 1:1 (keine Rundung) → 1.190,00 € brutto.
  await panel.getByRole("radio", { name: /Fester Betrag/ }).check();
  await panel.getByTestId("partial-amount").fill("1000");
  await panel.getByTestId("partial-invoice-submit").click();
  ...
  await expect(lines.getByText(/Teilbetrag zu /u)).toBeVisible();
  await expect(lines.getByText("1.190,00 €")).toBeVisible();

  // Zurück zur AB: Kette [Prozent, Betrag] + Rest 6.723,50 €
  // (11.305 − 3.391,50 − 1.190,00) sichtbar — Kette bleibt offen.
  await page.goto(orderConfirmationsPath());
  ...
  await expect(summary).toContainText("6.723,50 €");
  const chainList = page.getByRole("list", { name: "Gestellte Teilrechnungen" });
  await expect(chainList.getByText("Betrag 1.000,00 €").first()).toBeVisible();
