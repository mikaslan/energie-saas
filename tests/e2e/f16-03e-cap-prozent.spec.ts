...
function runtimeState(): M201RuntimeState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F16.3E-E2E-State ist unvollständig.");
  }
...
async function otpFromPrivateDevMailLog(
  ...
  throw new Error("Der echte F16.3E-Dev-Mail-OTP wurde nicht rechtzeitig protokolliert.");
}
...
function selectedVariantId(page: Page): string {
  ...
  if (!variantId) throw new Error("F16.3E-E2E-URL enthält keine aktive Variante.");
  return variantId;
}
...
test.afterEach(async ({ page }) => {
  expect(browserErrors.get(page) ?? [], "F16.3E Browser-Konsole und Page-Errors").toEqual([]);
});


test.describe("F16.3 Slice E Cap-Prozent global", () => {
  test("F16.3-E2E-05: Cap-Vorlage übernehmen, speichern, Total gedeckelt minus 10,00 €", async ({ page }) => {
    ...
    await creator.getByLabel("Name").fill("W3-E2E-Cap");
    await creator.getByLabel("Art").selectOption("percent_bps");
    await creator.getByLabel("Prozentsatz").fill("50");
    await creator.getByLabel("Deckel in Euro").fill("10");
    ...
    // 50 % von ~1,25 Mio. Cent >> Cap 1000 -> Cap bindet garantiert.
    expect(snapshot.globalDiscountBps).toBe(5000);
    expect(snapshot.globalDiscountCapCents).toBe(1000);
    expect(snapshot.totals.basisNetCents).toBe(before.totals.basisNetCents - 1000);
  });
});
