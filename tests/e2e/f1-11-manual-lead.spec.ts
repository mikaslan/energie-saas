...
/**
 * F1-11 Manuelle Anfrage — Chromium-E2E (isolierter Workspace).
 * Editor erfasst Name + Telefon + Adresse, die Karte erscheint auf dem
 * Wohnbau-Board, der Erfolgszustand verlinkt die neue Projektakte.
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "editorEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F1-11-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F1-11-E2E-01: Manuelle Anfrage landet als Karte auf dem Board", async ({ page }) => {
  ...
  await form.getByLabel("Name *").fill("E2E Manuell");
  ...
  const detailLink = success.getByRole("link", { name: "Projektakte öffnen" });
  await expect(detailLink).toBeVisible();
  await detailLink.click();
  await expect(page).toHaveURL(/\/anfragen\/[0-9a-f-]+$/u);

  expect(errors, "Browser-Konsole und Page-Errors der Erfassungs-Grenze").toEqual([]);
});
