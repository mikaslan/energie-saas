...
/**
 * F11-03a Notiz-Outbox — Chromium-E2E (isolierter Workspace).
 * Offline angelegte Notiz landet in der Outbox (statt Fehlschlag) und
 * wird online genau einmal synchronisiert (Replay-Guard, kein Duplikat).
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "editorEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F11-03a-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F11-03a-E2E-01: Offline-Notiz wird online genau einmal synchronisiert", async ({
  page,
  context,
}) => {
  ...
  await form.getByLabel("Name *").fill("E2E Notiz-Outbox");
  ...
  const noteText = `Offline-Entwurf ${stamp}`;
  ...
  await expect(notes.getByText("Offline gespeichert")).toBeVisible();

  await context.setOffline(false);
  await expect(notes.getByText("wurde synchronisiert")).toBeVisible({ timeout: 30_000 });
  await expect(notes.getByText(noteText, { exact: false })).toBeVisible();
  expect(await notes.getByText(noteText, { exact: false }).count()).toBe(1);
