...
/**
 * F9-08 Lauf-Auswertung — Chromium-E2E.
 * - Editor erfasst + freigegebenen Eintrag, schließt einen Tages-Lauf und
 *   öffnet die Aufschlüsselung je Person → eigene Zeile mit Einträgen/Minuten.
 */
...
const ENTRY_COMMENT = "F9-08 E2E Eintrag";
const RUN_LABEL = "F9-08 E2E Lauf";
const TYPE_NAME = "Auswertung-Montage";
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F9-08-E2E-State ist unvollständig.");
  }
...
async function firstProjectId(): Promise<string> {
  ...
  try {
    ...
    if (!result.rows[0]) throw new Error("Kein Projekt im F9-08-E2E-State vorhanden.");
...
test("F9-08-E2E-01: Editor schließt Tages-Lauf und sieht die Aufschlüsselung je Person", async ({ page }) => {
  ...
  await createForm.getByLabel("Beginn").fill("2026-09-15T08:00");
  await createForm.getByLabel("Ende").fill("2026-09-15T10:00");
  ...
  const entry = page.locator("li", { hasText: ENTRY_COMMENT }).first();
  ...
  await runs.getByLabel("Von").fill("2026-09-15");
  await runs.getByLabel("Bis").fill("2026-09-15");
  ...
  await runItem(page).getByText("Aufschlüsselung je Person", { exact: true }).click();
  await expect(runItem(page).getByText(`${data.editorEmail} — 1 Eintrag — 2 Std. 0 Min.`, { exact: true })).toBeVisible();
  expect(errors, "Browser-Konsole und Page-Errors der Lauf-Auswertung").toEqual([]);
});
