...
/**
 * F12-02 Kampagnen-Auto-Routing — Chromium-E2E (isolierter Workspace).
 * Editor legt per UI eine Kampagne MIT Beauftragtem an, erfasst danach
 * eine manuelle Anfrage mit dieser Kampagne; die Projektakte zeigt den
 * Beauftragten als Key Account (Stand 1). Regelvollzug, kein
 * Zuweisungsrecht des Erfassers nötig.
 */
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F12-02-E2E-State ist unvollständig.");
  }
...
async function seedIsolatedWorkspace(): Promise<{ workspaceId: string }> {
  ...
  try {
    ...
    if (!adminId) throw new Error("F12-02-E2E: Admin-Identität fehlt.");
    if (!editorId) throw new Error("F12-02-E2E: Editor-Identität fehlt.");
    await withTenantOn(pool, workspaceId, async (tx) => {
      await tx.execute(sql`
        insert into workspace (id, name) values (${workspaceId}::uuid, 'F12-02 isoliert')
      `);
      await tx.execute(sql`
      ...
      await tx.execute(sql`
        ...
        values (${workspaceId}::uuid, 'F1202 E2E Messe', 'f1202 e2e messe')
      `);
    });
    return { workspaceId };
  } finally {
...
test("F12-02-E2E-01: Kampagne mit Beauftragtem routet Anfrage auf Key Account", async ({ page }) => {
  ...
  await campaignForm.getByLabel("Name").fill("Routing-Messe");
  await campaignForm.getByLabel("Slug").fill("routing-2026");
  await campaignForm.getByLabel("Lead-Quelle").selectOption({ label: "F1202 E2E Messe" });
  await campaignForm.getByLabel(/Beauftragter/).selectOption({ label: data.editorEmail });
  ...
  await expect(page.getByText(`Zuweisung: ${data.editorEmail}`, { exact: false })).toBeVisible();

  await expectNoWcagAaAxeViolations(page, "F12-02-Einstellungsseite");
  ...
  await form.getByLabel("Name *").fill("Routing Kontakt");
  await form.getByLabel("E-Mail").fill("routing@f1202-e2e.test");
  await form.getByLabel(/Funnel-Kampagne/).selectOption({
    label: `Routing-Messe · F1202 E2E Messe · Zuweisung: ${data.editorEmail}`,
  });
  ...
  expect(errors, "F12-02 Browser-Konsole und Page-Errors").toEqual([]);
});
