...
/**
 * F9-15c projektlos Offline-Anlage — Chromium-E2E.
 * Bindung: `docs/spec/F9-15-projektlos-folge.md` (Track R1c).
 * Seed: EIGENER isolierter Workspace (NIEMALS W3).
 * Erwartung: offline Erfassen → „Offline gespeichert" → online Replay →
 * „wurde synchronisiert" + genau 1 Eintrag (Muster f11-03b).
 */
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F9-15c-E2E-State ist unvollständig.");
  }
...
async function otpFromPrivateDevMailLog(
  ...
  throw new Error("Der echte F9-15c-Dev-Mail-OTP wurde nicht rechtzeitig protokolliert.");
}
...
async function seedIsolatedWorkspace(): Promise<{ workspaceId: string }> {
  ...
  try {
    ...
    if (!editorId) throw new Error("F9-15c-E2E: Editor-Identität fehlt.");
    const client = await pool.connect();
    try {
      await client.query("insert into workspace (id, name) values ($1::uuid, $2)", [
        workspaceId,
        "F9-15c isolierter Offline-Workspace",
      ]);
      await client.query(
...
test("F915c-E2E-01: offline projektlos erfassen, online replayen, genau 1 Eintrag", async ({
  page,
  context,
}: {
  page: Page;
  ...
  const comment = `F915c Offline ${stamp}`;
  ...
  await expectNoWcagAaAxeViolations(page, "F9-15c-Offline");
});
