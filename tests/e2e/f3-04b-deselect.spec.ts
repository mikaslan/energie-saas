...
/**
 * F3-04b Einzelmodul-Abwahl (Stufe-0) — Chromium-E2E (TDD RED, Anker).
 *
 * vertrag: docs/spec/F3-04b-deselect.md
 * - Editor legt eine Panelgruppe per UI an (F3-04a-Testids
 *   planning-panel-groups-*), wählt 2 Zellen ab, der
 *   Effektiv-Count sinkt (24→22), Doppel-Abwahl ist
 *   idempotent (Count bleibt, kein neuer Eintrag), eine
 *   Out-of-Range-Abwahl (row 99) scheitert mit Fehler
 *   (kein neuer Eintrag), Re-Select lässt den Count
 *   wieder steigen.
 * - Viewer liest Abwahlen read-only (kein Formular, kein
 *   Anlegen, kein Löschen). External bleibt fail-closed
 *   (Sektion unsichtbar).
 *
 ...
 * - [data-testid="planning-panel-deselect-list"] /
 *   [data-testid="planning-panel-deselect-empty"]
 * - [data-testid="planning-panel-deselect-form"] (nur Editor)
 * - [data-testid="planning-panel-deselect-create"] /
 *   [data-testid="planning-panel-deselect-delete"] (je Listeneintrag)
 * - [data-testid="planning-panel-deselect-count"] (Text enthält
 *   den Effektiv-Count, z. B. "22")
 * - Formular-Controls (innerhalb der Form):
 *   [data-testid="planning-panel-deselect-group"] (<select>,
 *   Optionen = Gruppen-Labels),
 *   [data-testid="planning-panel-deselect-row"] (Zahl ≥1),
 *   [data-testid="planning-panel-deselect-col"] (Zahl ≥1),
 *   [data-testid="planning-panel-deselect-reason"] (optional,
 *   ≤280 Zeichen)
 *
 * - Listeneinträge zeigen Gruppe + Zeile/Spalte.
 * - Fehlschlag: [role="alert"] im Abwahl-Formular sichtbar,
 *   kein neuer Listeneintrag.
 * - Doppel-Abwahl: No-op (kein Reject) — Count bleibt,
 *   kein neuer Eintrag.
 *
 * Meldungen: "Abwahl gespeichert." / "Abwahl entfernt."
 *
 * Fixture-Hinweis: Wie F3-05b legt das Setup je Test eigene
 * Dächer (Meter-Rechteck 20x12 per DB-Rescale) + eigene
 * Gruppen an; Labels je Test eindeutig, serieller Modus,
 * ein Projekt. Raster 4x6 = 24 Module, Effektiv-Count
 * 24→22 nach 2 Abwahlen.
 */
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F3.4b-E2E-State ist unvollständig.");
  }
...
test.describe("F3-04b Einzelmodul-Abwahl — Browser-Gate", () => {
  test.describe.configure({ mode: "serial" });

  test("F304b-E2E-01: Editor wählt 2 Zellen ab → Count 24→22 → Doppel-Abwahl idempotent → Out-of-Range scheitert → Re-Select", async ({ page }) => {
    ...
    await createPanelGroup(page, "Desel-G1", "h");

    const section = page.getByTestId("planning-panel-deselect-section");
    await expect(section).toBeVisible();
    await expect(section.getByTestId("planning-panel-deselect-empty")).toBeVisible();
    await expect(section.getByTestId("planning-panel-deselect-count")).toContainText("24");

    await deselectCell(page, { groupLabel: "Desel-G1", row: 1, col: 1 });
    await deselectCell(page, {
      groupLabel: "Desel-G1",
      row: 1,
      col: 2,
      reason: "Verschattung",
    });
    await expect(section.getByTestId("planning-panel-deselect-count")).toContainText("22");
    await expect(section.getByTestId("planning-panel-deselect-delete")).toHaveCount(2);

    // Doppel-Abwahl: idempotent — Count bleibt, kein neuer Eintrag.
    await fillDeselectForm(page, { groupLabel: "Desel-G1", row: 1, col: 1 });
    await section.getByTestId("planning-panel-deselect-create").click();
    await expect(section.getByTestId("planning-panel-deselect-count")).toContainText("22");
    await expect(section.getByTestId("planning-panel-deselect-delete")).toHaveCount(2);

    // Out-of-Range: row 99 scheitert mit Fehler, kein neuer Eintrag.
    await fillDeselectForm(page, { groupLabel: "Desel-G1", row: 99, col: 1 });
    await section.getByTestId("planning-panel-deselect-create").click();
    await expect(
      section.getByTestId("planning-panel-deselect-form").getByRole("alert"),
    ).toBeVisible();
    await expect(section.getByTestId("planning-panel-deselect-delete")).toHaveCount(2);
    await expect(section.getByTestId("planning-panel-deselect-count")).toContainText("22");
    ...
    await page.reload();
    const reloaded = page.getByTestId("planning-panel-deselect-section");
    await expect(reloaded.getByTestId("planning-panel-deselect-delete")).toHaveCount(2);
    await expect(reloaded.getByTestId("planning-panel-deselect-count")).toContainText("22");

    // Re-Select: Count steigt wieder.
    await reloaded.getByTestId("planning-panel-deselect-delete").first().click();
    await expect(reloaded.getByText("Abwahl entfernt.", { exact: true })).toBeVisible();
    await expect(reloaded.getByTestId("planning-panel-deselect-count")).toContainText("23");
    await expect(reloaded.getByTestId("planning-panel-deselect-delete")).toHaveCount(1);

    await expectNoWcagAaAxeViolations(page, "F3.4b Abwahl-Sektion");
    expect(errors, "Browser-Konsole und Page-Errors des Abwahl-Flows").toEqual([]);
  });

  test("F304b-E2E-02: Viewer liest Abwahlen read-only", async ({ page }) => {
    test.setTimeout(180_000);
    const data = state();
    const errors = trackBrowserErrors(page);
    const path = projectPath();
    ...
    await page.goto(path);
    await loginWithRealOtp(page, data.editorEmail, path);
    await createPanelGroup(page, "DeselViewer-G1", "h");
    await deselectCell(page, { groupLabel: "DeselViewer-G1", row: 2, col: 3 });

    await page.context().clearCookies();
    await page.goto(path);
    await loginWithRealOtp(page, data.viewerEmail, path);
    const viewerSection = page.getByTestId("planning-panel-deselect-section");
    await expect(viewerSection).toBeVisible();
    await expect(
      viewerSection.getByTestId("planning-panel-deselect-list").getByText("DeselViewer-G1"),
    ).toBeVisible();
    await expect(viewerSection.getByTestId("planning-panel-deselect-count")).toContainText("23");
    await expect(viewerSection.getByTestId("planning-panel-deselect-form")).toHaveCount(0);
    await expect(viewerSection.getByTestId("planning-panel-deselect-create")).toHaveCount(0);
    await expect(viewerSection.getByTestId("planning-panel-deselect-delete")).toHaveCount(0);

    expect(errors, "Browser-Konsole und Page-Errors der Viewer-Sicht").toEqual([]);
  });

  test("F304b-E2E-03: External fail-closed (Sektion unsichtbar)", async ({ page }) => {
    test.setTimeout(180_000);
    const data = state();
    const errors = trackBrowserErrors(page);
    const path = projectPath();
