...
/**
 * F7-03C Platzhalter (Katalog F7.2) — Chromium-E2E (isolierter Workspace).
 * Admin schreibt {{kunde}}/{{datum}} in einen Punkt-Titel; Anzeige
 * substituiert, Eingabe zeigt Rohtext (Reload-fest); Viewer lesend.
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "adminEmail", "viewerEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F7-03C-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F7-03C-E2E-01: Platzhalter werden angezeigt substituiert, roh gespeichert", async ({ page }) => {
  ...
  const leadName = `E2E Platzhalter ${stamp}`;
  ...
  const rawTitle = "Abnahme {{kunde}} am {{datum}}";
  // Mitternachts-tolerant: Test-Uhr vs. Server-Uhr (beide Berlin) duerfen
  // an der Tagesgrenze um einen Tag abweichen — Name exakt, Datum Menge.
  const berlinDay = (shiftDays: number): string => {
    const at = new Date(Date.now() + shiftDays * 86_400_000);
  ...
  const shownTitlePattern = new RegExp(
    `^Abnahme ${leadName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} am (\\d{2}\\.\\d{2}\\.\\d{4})$`,
    "u",
  );
  const validDays = new Set([berlinDay(-1), berlinDay(0), berlinDay(1)]);
  const expectShownTitle = async (locator: ReturnType<Page["locator"]>): Promise<void> => {
    const name = await locator.getAttribute("aria-label");
    const match = name === null ? null : shownTitlePattern.exec(name);
    expect(match !== null, `Checkbox-Name substituiert (${name})`).toBe(true);
    expect(validDays.has(match![1]!), `Datum plausibel (${match![1]})`).toBe(true);
  };
  ...
  // Anzeige substituiert (Checkbox-Name), Eingabe zeigt Rohtext.
  const item = page.locator("li").filter({ has: page.getByLabel("Punkt-Name 1.1") });
  await expectShownTitle(item.getByRole("checkbox").first());
  ...
  await page.reload();
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  const reloaded = page.locator("li").filter({ has: page.getByLabel("Punkt-Name 1.1") });
  await expectShownTitle(reloaded.getByRole("checkbox").first());
  await expect(page.getByLabel("Punkt-Name 1.1")).toHaveValue(rawTitle);

  // Viewer (lesend): substituierter Text, kein Eingabefeld.
  await page.context().clearCookies();
  await page.goto(url);
  await loginWithRealOtp(page, data.viewerEmail, url);
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  const viewerText = (await page.locator("li", { hasText: `Abnahme ${leadName} am` })
    .first().textContent()) ?? "";
  const viewerMatch = new RegExp(
    `Abnahme ${leadName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} am (\\d{2}\\.\\d{2}\\.\\d{4})`,
    "u",
  ).exec(viewerText);
  expect(viewerMatch !== null, `Viewer-Text substituiert (${viewerText})`).toBe(true);
  expect(validDays.has(viewerMatch![1]!), `Viewer-Datum plausibel (${viewerMatch![1]})`).toBe(true);
  await expect(page.getByLabel("Punkt-Name 1.1")).toHaveCount(0);

  await expectNoWcagAaAxeViolations(page, "F7-03C-Platzhalter");

  expect(errors, "Browser-Konsole und Page-Errors der Platzhalter").toEqual([]);
});
