...
/**
 * F7-02B Bedingte Sichtbarkeit if/then — Chromium-E2E (isolierter Workspace).
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "adminEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F7-02B-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F702B-E2E-01: Bedingter Pflichtpunkt versteckt sich und erscheint wieder", async ({ page }) => {
  ...
  await form.getByLabel("Name *").fill("E2E Bedingte Sichtbarkeit");
  ...
  const referenceTitle = `Dachhaken vorhanden ${stamp}`;
  const conditionalTitle = `Gaube vermessen ${stamp}`;
  ...
  await page.getByLabel("Punkt-Name 1.1").fill(referenceTitle);
  await page.getByRole("button", { name: "Punkt hinzufügen" }).click();
  await page.getByLabel("Punkt-Name 1.2").fill(conditionalTitle);
  await page.getByLabel(`${conditionalTitle}: Pflichtpunkt`).check();
  ...
  // Titel rendern als Inputs (Admin editiert Struktur): An-/Abwesenheit
  // läuft über das Namens-Input, nicht über Text. Regel setzen versteckt
  // das Item samt Editor sofort — deshalb zuerst die Referenz erledigen.
  await page.getByLabel(referenceTitle, { exact: true }).check();
  const conditionalItem = page.locator("li").filter({ has: page.getByLabel("Punkt-Name 1.2") });
  await conditionalItem.getByLabel("Sichtbar, wenn").selectOption({ label: referenceTitle });
  await expect(conditionalItem.getByLabel(`${conditionalTitle}: Bedingung`)).toBeVisible();
  ...
  await page.getByLabel(referenceTitle, { exact: true }).uncheck();
  await expect(page.getByLabel("Punkt-Name 1.2")).toBeHidden();
  await expect(page.getByText("Noch 1 Pflichtpunkt offen.", { exact: true })).toHaveCount(0);
  await page.getByRole("button", { name: "Speichern" }).click();
  await expect(page.getByText("Gespeichert (Version 3).", { exact: true })).toBeVisible();
  ...
  await expectNoWcagAaAxeViolations(page, "F7-02B-Regel");
