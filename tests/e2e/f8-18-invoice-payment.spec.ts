...
import { INVOICE_PAYMENT_TEMPLATE_VERSION } from "../../lib/integrations/invoicing/pdf-contract";
import {
...
/**
 * F8-18 Zahlungsbeleg-PDF (EPC-QR) — Browser-Ketten (Chromium, Spiegel
 * M3-02d-E2E-Abschluss): ausgestellte Rechnung mit offenem Rest →
 * Zahlungsbeleg anfordern → synthetischer Worker-Abschluss → Status →
 * privater Download mit Byte-/Hash-Pruefung (`-zahlung.pdf`); vollbezahlte
 * Rechnung → Gating-Hinweis statt Button.
 */
...
const DOCUMENT_NUMBER = "F818-E2E-000001";
const DOCUMENT_SEQUENCE = 990081;
const SETTLED_DOCUMENT_NUMBER = "F818-E2E-000002";
const SETTLED_DOCUMENT_SEQUENCE = 990082;
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F8-18-E2E-State ist unvollständig.");
  }
...
async function seedPaymentFixture(options: {
  ...
  try {
    ...
    const document = await client.query<{ id: string }>(
      `insert into commercial_document (
         ...
         gen_random_uuid(), $1::uuid, 'invoice', 'draft', 'F818-E2E-Rechnung',
       ...
       returning id`,
      [data.workspaceId, data.editorEmail, options.documentNumber, options.documentSequence],
    );
    const documentId = document.rows[0]?.id;
    if (!documentId) throw new Error("F818-E2E-Seed: Dokument fehlt.");
    await client.query(
...
async function readRequestedPaymentJob(documentId: string): Promise<RequestedPdfJob> {
  return withInvoiceDatabase(async (tx) => {
    ...
    if (!job) throw new Error("Der angeforderte F8-18-E2E-Zahlungsauftrag fehlt.");
    return job;
  });
}
...
async function bytesFromDownload(download: Download): Promise<Buffer> {
  ...
  if (!path) throw new Error("Der F8-18-E2E-Download hat keinen Pfad.");
  return readFileSync(path);
}

test.describe("F8-18 Zahlungsbeleg-PDF", () => {
  test("F818-E2E-01: fordert im Browser an und prüft den synthetischen Abschluss bis zum privaten Download", async ({ page }) => {
    test.setTimeout(120_000);
    const data = state();
    const documentId = await seedPaymentFixture({
    ...
    const paymentPanel = page.locator("aside").filter({
      has: page.getByRole("heading", { name: "Zahlungsbeleg", exact: true }),
    });
    const invoicePanel = page.locator("aside").filter({
    ...
    await paymentPanel.getByRole("button", {
      name: "Zahlungsbeleg erzeugen",
      exact: true,
    }).click();
    expect((await generateResponsePromise).status()).toBe(200);
    await expect(paymentPanel.getByText(
      "Der Zahlungsbeleg-Auftrag für das Dokument wurde angenommen.",
      { exact: true },
    )).toBeVisible();
    await expect(paymentPanel.getByText("Angefordert", { exact: true })).toBeVisible();

    const requested = await readRequestedPaymentJob(documentId);
    ...
    if (claim === null) throw new Error("Der requested F8-18-E2E-Auftrag war nicht claimbar.");
    ...
    await page.reload();
    await expect(paymentPanel.getByText(
      "Zahlungsbeleg ist bereit",
      { exact: true },
    )).toBeVisible();
    // Track-Partition: Der Payment-Job erscheint nicht als Rechnungs-PDF.
    await expect(invoicePanel.getByText(
      "Noch kein Rechnungs-PDF vorhanden.",
      { exact: true },
    )).toBeVisible();
    const downloadLink = paymentPanel.getByRole("link", {
      name: "Zahlungsbeleg laden",
      exact: true,
    });
    ...
    const expectedFilename = `${DOCUMENT_NUMBER}-zahlung.pdf`;
  ...
  test("F818-E2E-02: vollbezahlter Beleg zeigt Gating-Hinweis statt Button", async ({ page }) => {
    test.setTimeout(120_000);
    const data = state();
    const documentId = await seedPaymentFixture({
      documentNumber: SETTLED_DOCUMENT_NUMBER,
      documentSequence: SETTLED_DOCUMENT_SEQUENCE,
      settled: true,
    });
    const detailPath = `/w/${data.workspaceId}/rechnungen/invoice/${documentId}`;
    await page.goto(detailPath);
    ...
    await expect(paymentPanel.getByRole("button", {
      name: "Zahlungsbeleg erzeugen",
      exact: true,
    })).toHaveCount(0);
  });
