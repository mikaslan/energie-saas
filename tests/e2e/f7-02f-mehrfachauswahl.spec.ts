...
/**
 * F7-02F Mehrfachauswahl (Katalog F7.2, Slice B) — Chromium-E2E
 * (isolierter Workspace). Admin legt zwei Punkte an, stellt beide auf Typ
 * „Mehrfachauswahl", wählt beide an: BEIDE bleiben gewählt (keine
 * Exklusivität — Gegenstück zu F7-02D). Speichern + Reload belegt die
 * Persistenz; Viewer sieht deaktivierte Checkboxen.
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "adminEmail", "viewerEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F7-02F-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F7-02F-E2E-01: Mehrfachauswahl bleibt je Segment mehrfach wählbar und persistent", async ({ page }) => {
  ...
  await form.getByLabel("Name *").fill("E2E Mehrfach-Auswahl");
  ...
  const firstTitle = `Speicher A ${stamp}`;
  const secondTitle = `Speicher B ${stamp}`;
  ...
  await firstItem.getByLabel("Typ").selectOption("multi");
  ...
  await secondItem.getByLabel("Typ").selectOption("multi");

  // Ehrliches Umschreiben: erledigte Aufgabe → Multi behält den Haken
  // (Doppel-done ist speicherbar — anders als Radio, das zurückfällt).
  await page.getByRole("button", { name: "Punkt hinzufügen" }).click();
  const thirdTitle = `Speicher C ${stamp}`;
  ...
  await thirdItem.getByLabel("Typ").selectOption("multi");
  await expect(thirdItem.getByRole("checkbox", { name: thirdTitle, exact: true })).toBeChecked();

  // Pflicht-Gate zählt den offenen Multi-Punkt wie eine Aufgabe.
  await expect(page.getByText("Noch 1 Pflichtpunkt offen.", { exact: true })).toBeVisible();

  const firstBox = firstItem.getByRole("checkbox", { name: firstTitle, exact: true });
  const secondBox = secondItem.getByRole("checkbox", { name: secondTitle, exact: true });
  ...
  // KEINE Exklusivität: die erste Auswahl bleibt — Gegenstück zu Radio.
  await expect(firstBox).toBeChecked();
  ...
  await expect(firstReloaded.getByRole("checkbox", { name: firstTitle, exact: true })).toBeChecked();
  await expect(secondReloaded.getByRole("checkbox", { name: secondTitle, exact: true })).toBeChecked();
  await expect(page.getByText("Punkte: 3/3", { exact: true })).toBeVisible();

  // Viewer (lesend): Checkboxen sichtbar, aber deaktiviert, ohne Typ-Editor.
  await page.context().clearCookies();
  await page.goto(url);
  await loginWithRealOtp(page, data.viewerEmail, url);
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  const viewerFirst = page.locator("li", { hasText: firstTitle });
  await expect(viewerFirst.getByRole("checkbox", { name: firstTitle, exact: true })).toBeChecked();
  await expect(viewerFirst.getByRole("checkbox", { name: firstTitle, exact: true })).toBeDisabled();
  await expect(viewerFirst.getByLabel("Typ")).toHaveCount(0);
  await expect(page.getByText("Punkte: 3/3", { exact: true })).toBeVisible();

  await expectNoWcagAaAxeViolations(page, "F7-02F-Multi");

  expect(errors, "Browser-Konsole und Page-Errors der Mehrfachauswahl").toEqual([]);
});
