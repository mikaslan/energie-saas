...
/**
 * F7-02H Block-Fälligkeitsdatum (Katalog F7.2) — Chromium-E2E (isolierter
 * Workspace). Admin setzt ein Block-Datum, speichert (Reload-fest);
 * Viewer sieht das Datum lesend.
 */
...
function state(): E2EState {
  ...
  for (const key of ["baseURL", "databaseUrl", "serverLogPath", "adminEmail", "viewerEmail"] as const) {
    if (typeof full[key] !== "string" || full[key] === "") {
      throw new Error(`Der private F7-02H-E2E-State ist unvollständig (${key}).`);
    }
  }
...
test("F7-02H-E2E-01: Block-Fälligkeitsdatum ist persistent und lesbar", async ({ page }) => {
  ...
  await form.getByLabel("Name *").fill("E2E Block-Datum");
  ...
  await page.getByLabel("Block 1: Fälligkeitsdatum").fill("2026-10-01");
  ...
  await page.getByLabel("Punkt-Name 1.1").fill("Dachfläche");
  ...
  await page.reload();
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  await expect(page.getByLabel("Block 1: Fälligkeitsdatum")).toHaveValue("2026-10-01");

  // Leeren = Datum entfernen (Version 2).
  await page.getByLabel("Block 1: Fälligkeitsdatum").fill("");
  await page.getByRole("button", { name: "Speichern" }).click();
  await expect(page.getByText("Gespeichert (Version 2).", { exact: true })).toBeVisible();
  await expect(page.getByLabel("Block 1: Fälligkeitsdatum")).toHaveValue("");

  // Erneut setzen fuer die Viewer-Sicht (Version 3).
  await page.getByLabel("Block 1: Fälligkeitsdatum").fill("2026-12-24");
  await page.getByRole("button", { name: "Speichern" }).click();
  await expect(page.getByText("Gespeichert (Version 3).", { exact: true })).toBeVisible();

  // Viewer (lesend): Datum als Text, kein Eingabefeld.
  await page.context().clearCookies();
  await page.goto(url);
  await loginWithRealOtp(page, data.viewerEmail, url);
  await expect(page.getByRole("heading", { name: "Checkliste", level: 1 })).toBeVisible();
  await expect(page.getByText("Fällig: 24.12.2026", { exact: true })).toBeVisible();
  await expect(page.getByLabel("Block 1: Fälligkeitsdatum")).toHaveCount(0);

  await expectNoWcagAaAxeViolations(page, "F7-02H-Datum");

  expect(errors, "Browser-Konsole und Page-Errors des Block-Datums").toEqual([]);
});
