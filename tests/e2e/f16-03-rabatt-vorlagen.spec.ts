...
/**
 * F16.3 Slice A Rabatt-Vorlagen — Chromium-E2E (Welle 03/04).
 *
...
function state(): E2EState {
  ...
  if (required.some((key) => typeof parsed[key] !== "string" || parsed[key] === "")) {
    throw new Error("Der private F16.3-E2E-State ist unvollständig.");
  }
...
test("F16.3-E2E-01: Fix- und Prozent-Vorlage anlegen und sehen", async ({ page }) => {
  ...
  const url = `/w/${data.w3WorkspaceId}/einstellungen/rabatt-vorlagen`;
  await page.goto(url);
  ...
  await creator.getByRole("button", { name: "Anlegen", exact: true }).click();
  await expect(page.getByText("W3-Prozent-Rabatt", { exact: true })).toBeVisible();
