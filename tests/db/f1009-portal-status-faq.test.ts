// F10-09 Portal-FAQ je Installationsstand (PostgreSQL):
// Upsert/Liste/Reset + Portal-Projektion der Overrides.
...
async function seedFixture(emailDomain: string): Promise<Fixture> {
  ...
  await withTenantOn(testPool, workspaceId, async (tx) => {
    await tx.execute(sql`insert into workspace (id, name) values (${workspaceId}::uuid, 'F1009 FAQ')`);
    ...
    await tx.execute(sql`
      ...
      values (${contactId}::uuid, ${workspaceId}::uuid, 'F1009', 'F', 'X', 'c@f1009.test', 'c@f1009.test')
    `);
    await tx.execute(sql`
      insert into site (id, workspace_id, contact_id, label)
      values (${siteId}::uuid, ${workspaceId}::uuid, ${contactId}::uuid, 'F1009 Site')
    `);
    await tx.execute(sql`
      ...
      select ${projectId}::uuid, ${workspaceId}::uuid, ${contactId}::uuid, ${siteId}::uuid,
             board.id, intake.id, 'F1009 Project', 'manual'
...
describe("F10-09 Portal-FAQ je Installationsstand (PostgreSQL)", () => {
  let fixture: Fixture;

  beforeEach(async () => {
    fixture = await seedFixture("f1009.test");
  });
  ...
  it("F1009-DB-01: Upsert-Roundtrip, Reset, ungemappt = null", async () => {
    expect(await asEditor((tx, ctx) => listInstallationStatusFaq(tx, ctx))).toEqual({
      active: null,
      completed: null,
      handover: null,
    });
    const afterSet = await asEditor((tx, ctx) =>
      upsertInstallationStatusFaq(tx, ctx, { key: "active", faq: "Die Montage läuft planmäßig." }));
    expect(afterSet).toEqual({ active: "Die Montage läuft planmäßig.", completed: null, handover: null });
    // Gleicher Schlüssel überschreibt (kein Duplikat).
    await asEditor((tx, ctx) =>
      upsertInstallationStatusFaq(tx, ctx, { key: "active", faq: "Montage fast fertig." }));
    const again = await asEditor((tx, ctx) => listInstallationStatusFaq(tx, ctx));
    expect(again.active).toBe("Montage fast fertig.");
    const count = await withTenantOn(testPool, fixture.workspaceId, async (tx) => {
      const rows = await tx.execute<{ n: string }>(sql`
        select count(*)::text as n from portal_status_faq
         where workspace_id = ${fixture.workspaceId}::uuid`);
      return Number(rows.rows[0]?.n ?? "0");
    });
    expect(count).toBe(1);
    const afterReset = await asEditor((tx, ctx) =>
      resetInstallationStatusFaq(tx, ctx, { key: "active" }));
    expect(afterReset).toEqual({ active: null, completed: null, handover: null });
  });

  it("F1009-DB-02: Validierung fail-closed, Viewer read-only, Mandantentrennung", async () => {
    // Unbekannter Schlüssel, Leertext, zu lang, Steuerzeichen.
    for (const input of [
      { key: "flying", faq: "X" },
      { key: "active", faq: "   " },
      { key: "active", faq: "A".repeat(2001) },
      { key: "active", faq: "Bau\tmorgen" },
    ]) {
      await expect(asEditor((tx, ctx) =>
        upsertInstallationStatusFaq(tx, ctx, input))).rejects.toBeInstanceOf(InstallationValidationError);
    }
    await expect(asEditor((tx, ctx) =>
      resetInstallationStatusFaq(tx, ctx, { key: "flying" }))).rejects.toBeInstanceOf(
      InstallationValidationError,
    );
    // Viewer liest, schreibt nicht.
    expect(await asViewer((tx, ctx) => listInstallationStatusFaq(tx, ctx))).toEqual({
      active: null,
      completed: null,
      handover: null,
    });
    await expect(asViewer((tx, ctx) =>
      upsertInstallationStatusFaq(tx, ctx, { key: "active", faq: "X" }))).rejects.toBeInstanceOf(
      PermissionDeniedError,
    );
    // Fremdmandant sieht nichts und schreibt getrennt.
    const foreign = await seedFixture("f1009-foreign.test");
    const runForeign = asEditorIn(foreign);
    expect(await runForeign((tx, ctx) => listInstallationStatusFaq(tx, ctx))).toEqual({
      active: null,
      completed: null,
      handover: null,
    });
    await runForeign((tx, ctx) =>
      upsertInstallationStatusFaq(tx, ctx, { key: "active", faq: "Fremde FAQ." }));
    expect(await asEditor((tx, ctx) => listInstallationStatusFaq(tx, ctx))).toEqual({
      active: null,
      completed: null,
      handover: null,
  ...
  it("F1009-DB-03: Resolver projiziert Overrides, ohne Mapping leeres Objekt", async () => {
    ...
    expect(empty.installation?.statusFaq).toEqual({});

    await asEditor((tx, ctx) =>
      upsertInstallationStatusFaq(tx, ctx, { key: "active", faq: "Die Montage läuft planmäßig." }));
    const mapped = await resolvePortalByToken(testPool, { token: invite.token });
    expect(mapped.installation?.statusFaq).toEqual({ active: "Die Montage läuft planmäßig." });
  });
});
