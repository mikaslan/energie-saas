...
/**
 * F7-02J Komponentenlisten-Punkt (Katalog F7.2) — kind=component-list als
 * Anzeige-Art (weder Pflicht noch abhakbar, keine Nutzlast). Validator 0177.
 * Fixture-Muster nach F7-03D (Projekt + Katalog-Komponente fuer DB-04).
 */
...
async function seedFixture(): Promise<Fixture> {
  ...
  await withTenantOn(testPool, workspaceId, async (tx) => {
    await tx.execute(sql`insert into workspace (id, name) values (${workspaceId}::uuid, 'F7-02J Komponentenliste')`);
    await tx.execute(sql`
      insert into user_identity (id, email)
      values (${editorId}::uuid, ${`editor-${editorId}@f702j.test`}),
             (${adminId}::uuid, ${`admin-${adminId}@f702j.test`})
    `);
    await tx.execute(sql`
    ...
    await tx.execute(sql`
      ...
      values (${contactId}::uuid, ${workspaceId}::uuid, 'F702J', 'F7', 'Fixture',
        ${`${contactId}@f702j.test`}, ${`${contactId}@f702j.test`})
    `);
    await tx.execute(sql`
      insert into site (id, workspace_id, contact_id, label)
      values (${siteId}::uuid, ${workspaceId}::uuid, ${contactId}::uuid, 'F702J Site')
    `);
    await tx.execute(sql`
      insert into project (
...
function componentListBlocks(itemId: string): EditableChecklistBlocksV2 {
  return [{
    id: randomUUID(),
    name: "PV",
    position: 0,
...
function patchedBlocks(
  ...
  const blocks = componentListBlocks(itemId);
...
describe("F7-02J Komponentenlisten-Punkt (PostgreSQL)", () => {
  let fixture: Fixture;
  beforeEach(async () => {
    fixture = await seedFixture();
  });

  it("F702J-DB-01: component-list persistiert (Save + Re-Read)", async () => {
    const itemId = randomUUID();
    const created = await saveAs(fixture, fixture.adminId, patchedBlocks(itemId, {
      kind: "component-list",
    }));
    expect(created.version).toBe(1);
    expect(created.blocks[0]!.segments[0]!.items[0]!.kind).toBe("component-list");
    ...
    expect(rereadItem.kind).toBe("component-list");
    expect(rereadItem.done).toBe(false);
    expect(rereadItem.required).toBe(false);
    expect(await directValid(fixture, patchedBlocks(itemId, { kind: "component-list" })))
      .toBe(true);
  });

  it("F702J-DB-02: required/done am kind werden verworfen", async () => {
    const itemId = randomUUID();
    for (const patch of [
      { kind: "component-list", required: true },
      { kind: "component-list", done: true },
    ]) {
  ...
  it("F702J-DB-03: fremder kind bleibt verworfen (IN-Regression)", async () => {
    const itemId = randomUUID();
  ...
  it("F702J-DB-04: Template-Anwendung erzeugt kind-Punkt mit done/required false", async () => {
    ...
    expect(applied.kind).toBe("component-list");
