...
/**
 * F7-02K Datenblatt-Punkt (Katalog F7.2) — kind=datasheets als Anzeige-Art
 * (weder Pflicht noch abhakbar, keine Nutzlast). Validator 0178.
 * Fixture-Muster nach F7-02J (Projekt + Katalog-Komponente fuer DB-04).
 */
...
async function seedFixture(): Promise<Fixture> {
  ...
  await withTenantOn(testPool, workspaceId, async (tx) => {
    await tx.execute(sql`insert into workspace (id, name) values (${workspaceId}::uuid, 'F7-02K Datenblaetter')`);
    await tx.execute(sql`
      insert into user_identity (id, email)
      values (${editorId}::uuid, ${`editor-${editorId}@f702k.test`}),
             (${adminId}::uuid, ${`admin-${adminId}@f702k.test`})
    `);
    await tx.execute(sql`
    ...
    await tx.execute(sql`
      ...
      values (${contactId}::uuid, ${workspaceId}::uuid, 'F702K', 'F7', 'Fixture',
        ${`${contactId}@f702k.test`}, ${`${contactId}@f702k.test`})
    `);
    await tx.execute(sql`
      insert into site (id, workspace_id, contact_id, label)
      values (${siteId}::uuid, ${workspaceId}::uuid, ${contactId}::uuid, 'F702K Site')
    `);
    await tx.execute(sql`
      insert into project (
...
function datasheetBlocks(itemId: string): EditableChecklistBlocksV2 {
  return [{
    id: randomUUID(),
    name: "PV",
    position: 0,
...
function patchedBlocks(
  ...
  const blocks = datasheetBlocks(itemId);
...
describe("F7-02K Datenblatt-Punkt (PostgreSQL)", () => {
  let fixture: Fixture;
  beforeEach(async () => {
    fixture = await seedFixture();
  });

  it("F702K-DB-01: datasheets persistiert (Save + Re-Read)", async () => {
    const itemId = randomUUID();
    const created = await saveAs(fixture, fixture.adminId, patchedBlocks(itemId, {
      kind: "datasheets",
    }));
    expect(created.version).toBe(1);
    expect(created.blocks[0]!.segments[0]!.items[0]!.kind).toBe("datasheets");
    ...
    expect(rereadItem.kind).toBe("datasheets");
    expect(rereadItem.done).toBe(false);
    expect(rereadItem.required).toBe(false);
    expect(await directValid(fixture, patchedBlocks(itemId, { kind: "datasheets" })))
      .toBe(true);
  });

  it("F702K-DB-02: required/done am kind werden verworfen", async () => {
    const itemId = randomUUID();
    for (const patch of [
      { kind: "datasheets", required: true },
      { kind: "datasheets", done: true },
    ]) {
  ...
  it("F702K-DB-03: fremder kind bleibt verworfen (IN-Regression)", async () => {
    const itemId = randomUUID();
  ...
  it("F702K-DB-04: Template-Anwendung erzeugt kind-Punkt mit done/required false", async () => {
    ...
    expect(applied.kind).toBe("datasheets");
