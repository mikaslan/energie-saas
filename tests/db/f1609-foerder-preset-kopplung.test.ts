...
import { createDiscountTemplate } from "@/modules/discounts";
import { archiveSubsidyTemplate, createSubsidyTemplate } from "@/modules/subsidies";
import {
  applyOfferTemplate,
  createOfferTemplate,
...
import { SubsidyTemplateNotFoundError } from "@/modules/subsidies";
...
async function createOfferMembers(): Promise<OfferMembers> {
  ...
  await withTenantOn(testPool, members.workspaceId, async (tx) => {
    await tx.execute(sql`
      ...
      values (${members.workspaceId}::uuid, 'F16-09 Angebots-Vorlagen')
    `);
    for (const userId of [
      members.operatorId,
      ...
      await tx.execute(sql`
        ...
        values (${userId}::uuid, ${`${userId}@f1609.test`})
      `);
    }
    await tx.execute(sql`
  ...
  await withTenantOn(testPool, members.workspaceId, async (tx) => {
    await tx.execute(sql`
      insert into contact (
        ...
        'Synthetischer F16-09-Kontakt', 'Fixture', 'Contact', 'f1609.fixture@example.test',
        'f1609.fixture@example.test'
      )
    `);
    await tx.execute(sql`
      insert into site (
        ...
        ${ids.contactId}::uuid, 'Synthetischer F16-09-Standort',
    ...
    await tx.execute(sql`
      ...
      select ${ids.projectId}::uuid, ${members.workspaceId}::uuid,
             ...
             board.id, intake.id, 'Synthetisches F16-09-Projekt', 'wmee-rechner-v3'
...
function productCommand(
  ...
  return {
    ...
    internalSku: `F1609-${type.toUpperCase()}-${index}`,
    ...
    commercial: {
      ...
      purchaseProvenance: {
        ...
        reference: `PRIVATE-F1609-PURCHASE-${type}-${index}`,
      ...
      salesProvenance: {
        ...
        reference: `SYNTHETIC-F1609-SALES-${type}-${index}`,
        observedOn: "2026-08-30",
    ...
    technicalProvenance: {
      ...
      reference: `SYNTHETIC-F1609-TECH-${type}-${index}`,
      observedOn: "2026-08-30",
      rightsBasis: "workspace_owned",
      sourceDocumentSha256: null,
    },
...
async function createSubsidyPreset(
  members: OfferMembers,
  kind: "percent_bps" | "fix_cents",
): Promise<string> {
  const created = await withAuthorizedTenantOn(
    testPool, members.operatorId, members.workspaceId,
    (tx, ctx) => createSubsidyTemplate(tx, ctx, {
      schemaVersion: SUBSIDY_TEMPLATE_SCHEMA_VERSION,
      name: kind === "percent_bps" ? "F1609-Drei-Prozent" : "F1609-Hundert-Euro",
      kind,
      amountCents: kind === "fix_cents" ? 10_000 : null,
      percentBps: kind === "percent_bps" ? 300 : null,
      capCents: null,
    }),
  );
  return created.id;
...
async function createOfferPreset(
  members: OfferMembers,
  input: { name: string; paymentOptionId?: string | null; discountTemplateId?: string | null; subsidyTemplateId?: string | null },
): Promise<string> {
  const created = await withAuthorizedTenantOn(
    testPool, members.operatorId, members.workspaceId,
    (tx, ctx) => createOfferTemplate(tx, ctx, {
      ...
      subsidyTemplateId: input.subsidyTemplateId ?? null,
    }),
  );
  return created.id;
}
...
async function readVariantState(
  ...
  variantId: string,
  revision: number,
): Promise<{ bps: number | null; fixCents: number | null; paymentOptionId: string | null }> {
  return withTenantOn(testPool, workspaceId, async (tx) => {
    const result = await tx.execute<{ bps: number | null; fix_cents: number | null; payment_option_id: string | null }>(sql`
      select (revision_snapshot ->> 'globalDiscountBps')::integer as bps,
             (revision_snapshot ->> 'globalFixDiscountCents')::integer as fix_cents,
    ...
    return { bps: row.bps, fixCents: row.fix_cents, paymentOptionId: row.payment_option_id };
  });
}

describe("F16-09 Förder-Preset-Kopplung (PostgreSQL)", () => {
  it("F1609-DB-01: Förder-only (Prozent) anlegen → anwenden setzt Slot + Flag + Revision", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const subsidyTemplateId = await createSubsidyPreset(members, "percent_bps");
    const templateId = await createOfferPreset(members, {
      name: "Nur Förderung 3 %",
      subsidyTemplateId,
    });
    ...
    expect(created.subsidyTemplateId).toBe(subsidyTemplateId);
    expect(created.discountTemplateId).toBeNull();
    expect(created.paymentOptionId).toBeNull();
    ...
    expect(applied.subsidyApplied).toBe(true);
    expect(applied.discountApplied).toBe(false);
    expect(applied.paymentOptionApplied).toBe(false);
    expect(applied.revision).toBe(2);

    const state = await readVariantState(members.workspaceId, offerId, variantId, 2);
    expect(state.bps).toBe(300);
    expect(state.fixCents).toBeNull();

    // Veraltete Revision mit Förder-Anteil → Konflikt, keine dritte Revision.
  ...
  it("F1609-DB-02: Fix-Förderung + Prozent-Rabatt koexistieren (getrennte Slots, eine Revision)", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const discountTemplateId = await createDiscountPreset(members);
    const subsidyTemplateId = await createSubsidyPreset(members, "fix_cents");
    const templateId = await createOfferPreset(members, {
      name: "Rabatt 5 % + Förderung 100 €",
      discountTemplateId,
      subsidyTemplateId,
    });
    ...
    expect(applied.subsidyApplied).toBe(true);
    expect(applied.paymentOptionApplied).toBe(false);
    // Rabatt + Förderung in EINER Revision (DB-Invariant: eine Revision je Tx).
    expect(applied.revision).toBe(2);

    const state = await readVariantState(members.workspaceId, offerId, variantId, 2);
    expect(state.bps).toBe(500);
    expect(state.fixCents).toBe(10_000);
  });

  it("F1609-DB-03: Prozent + Prozent teilen den Slot — Förderung gewinnt (dokumentierte Ordnung)", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const discountTemplateId = await createDiscountPreset(members);
    const subsidyTemplateId = await createSubsidyPreset(members, "percent_bps");
    const templateId = await createOfferPreset(members, {
      name: "Rabatt 5 % + Förderung 3 %",
      discountTemplateId,
      subsidyTemplateId,
    });
  ...
  it("F1609-DB-04: leer/fremd/mandantenfremd fail-closed; archivierte Förderung nicht anwendbar", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const subsidyTemplateId = await createSubsidyPreset(members, "percent_bps");

    // Nur-Förderung ist ein gültiges Preset (kein „leer").
    const onlySubsidy = await createOfferPreset(members, {
      name: "Nur Förderung",
      subsidyTemplateId,
    });
    expect(typeof onlySubsidy).toBe("string");

    // Gar kein Preset belegt.
    await expect(withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => createOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        ...
        subsidyTemplateId: null,
      }),
    )).rejects.toBeInstanceOf(OfferTemplateValidationError);

    // Unbekannte Förder-Referenz.
    await expect(createOfferPreset(members, {
      name: "Fremde Förderung",
      subsidyTemplateId: randomUUID(),
    })).rejects.toBeInstanceOf(OfferTemplateValidationError);

    // Mandantenfremde Förder-Referenz (zweiter Workspace).
    const other = await createOfferMembers();
    const foreignSubsidy = await createSubsidyPreset(other, "percent_bps");
    await expect(createOfferPreset(members, {
      name: "Fremder Mandant",
      subsidyTemplateId: foreignSubsidy,
    })).rejects.toBeInstanceOf(OfferTemplateValidationError);

    // Viewer darf keine Vorlage mit Förder-Preset anlegen.
    await expect(withAuthorizedTenantOn(
    ...
    // Archivierte Förderung ist referenzierbar, aber nicht anwendbar.
    const staleTemplate = await createOfferPreset(members, {
      name: "Stille Förderung",
      subsidyTemplateId,
    });
    await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => archiveSubsidyTemplate(tx, ctx, subsidyTemplateId),
    );
    await expect(withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => applyOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        templateId: staleTemplate,
        offerId,
        variantId,
        expectedRevision: 1,
      }),
