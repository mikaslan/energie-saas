...
import { PACKAGE_TEMPLATE_SCHEMA_VERSION } from "@/lib/integrations/offers/package-contract";
 ...
 * Paket = Sektion + freie Positionen; Anwenden ersetzt die
 * Custom-Ebene der Variante (Revise-Ops, expectedRevision),
 * Katalog-Seed-Zeilen bleiben unangetastet. Migration 0148.
 */
...
async function createOfferMembers(): Promise<OfferMembers> {
  ...
  await withTenantOn(testPool, members.workspaceId, async (tx) => {
    await tx.execute(sql`
      ...
      values (${members.workspaceId}::uuid, 'F16-11 Paket-Vorlagen')
    `);
    for (const userId of [members.operatorId, members.viewerId]) {
      await tx.execute(sql`
        insert into user_identity (id, email)
        values (${userId}::uuid, ${`${userId}@f1611.test`})
      `);
    }
    await tx.execute(sql`
  ...
  await withTenantOn(testPool, members.workspaceId, async (tx) => {
    await tx.execute(sql`
      insert into contact (
        ...
        'Synthetischer F16-11-Kontakt', 'Fixture', 'Contact', 'f1611.fixture@example.test',
        'f1611.fixture@example.test'
      )
    `);
    await tx.execute(sql`
      insert into site (
        ...
        ${ids.contactId}::uuid, 'Synthetischer F16-11-Standort',
    ...
    await tx.execute(sql`
      ...
      select ${ids.projectId}::uuid, ${members.workspaceId}::uuid,
             ...
             board.id, intake.id, 'Synthetisches F16-11-Projekt', 'wmee-rechner-v3'
...
function productCommand(
  ...
  return {
    ...
    internalSku: `F1611-${type.toUpperCase()}-${index}`,
    ...
    commercial: {
      ...
      purchaseProvenance: {
        ...
        reference: `PRIVATE-F1611-PURCHASE-${type}-${index}`,
      ...
      salesProvenance: {
        ...
        reference: `SYNTHETIC-F1611-SALES-${type}-${index}`,
        observedOn: "2026-08-30",
    ...
    technicalProvenance: {
      ...
      reference: `SYNTHETIC-F1611-TECH-${type}-${index}`,
      observedOn: "2026-08-30",
      rightsBasis: "workspace_owned",
      sourceDocumentSha256: null,
    },
...
async function createBasisOffer(): Promise<PackageOffer> {
...
async function readSections(
  ...
  return withTenantOn(testPool, workspaceId, async (tx) => {
    ...
    return row.sections as SnapshotSection[];
  });
}

async function createPackage(
  members: OfferMembers,
  input: { name: string; sectionTitle?: string; category?: "other"; lineCount?: number },
): Promise<string> {
  const count = input.lineCount ?? 2;
  const created = await withAuthorizedTenantOn(
    ...
    (tx, ctx) => createPackageTemplate(tx, ctx, {
      ...
      name: input.name,
      sectionTitle: input.sectionTitle ?? "Paket Sektion",
      category: input.category ?? "other",
      lines: Array.from({ length: count }, (_, i) => ({
        displayName: `Paketzeile ${i + 1}`,
        description: null,
        unit: "piece" as const,
        quantityMilli: (i + 1) * 1_000,
...
async function addCustomLine(
  ...
  sectionTitle: string,
): Promise<{ sectionDomainId: string; lineDomainId: string; revision: number }> {
  // Sektion + Zeile in einer Revision (Sektionen brauchen min. 1 Zeile).
  ...
  return { sectionDomainId, lineDomainId, revision: result.revision };
}

describe("F16-11 Paket-Vorlagen (PostgreSQL)", () => {
  it("F1611-DB-01: Anlage → Liste → Anwenden ersetzt Custom-Ebene, Katalog bleibt", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const templateId = await createPackage(members, { name: "Standardpaket" });

    const listed = await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => listPackageTemplates(tx, ctx).then((rows) => rows.find((row) => row.id === templateId)!),
    );
    expect(listed.name).toBe("Standardpaket");
    expect(listed.sectionTitle).toBe("Paket Sektion");
    expect(listed.lines).toHaveLength(2);
    expect(listed.active).toBe(true);
    expect(listed.permissions.canWrite).toBe(true);

    const viewerList = await withAuthorizedTenantOn(
      testPool, members.viewerId, members.workspaceId,
      (tx, ctx) => listPackageTemplates(tx, ctx),
    );
    expect(viewerList).toHaveLength(1);
    expect(viewerList[0]!.permissions.canWrite).toBe(false);

    // Vorzustand: Katalog-Seed (2 Zeilen) + eine freie Zeile.
    const seeded = await addCustomLine(members, offerId, variantId, 1, "Alte Sektion");
    const before = await readSections(members.workspaceId, offerId, variantId, seeded.revision);
    const catalogBefore = before.flatMap((s) => s.lines).filter((l) => l.source.kind === "catalog");
    expect(catalogBefore.length).toBeGreaterThan(0);
    expect(before.flatMap((s) => s.lines).some((l) => l.lineDomainId === seeded.lineDomainId)).toBe(true);
    ...
    expect(applied.addedLines).toBe(2);
    expect(applied.removedLines).toBe(1);
    expect(applied.removedSections).toBe(1);
    ...
    const linesAfter = after.flatMap((s) => s.lines);
    // Alte freie Zeile ist ersetzt, Katalog-Zeilen überleben.
    expect(linesAfter.some((l) => l.lineDomainId === seeded.lineDomainId)).toBe(false);
    for (const catalogLine of catalogBefore) {
      expect(linesAfter.some((l) => l.lineDomainId === catalogLine.lineDomainId)).toBe(true);
    }
    // Paket-Sektion mit beiden Zeilen am Ende.
    const packageSection = after[after.length - 1]!;
    expect(packageSection.title).toBe("Paket Sektion");
    expect(packageSection.lines).toHaveLength(2);
    expect(packageSection.lines.map((l) => l.product.displayName)).toEqual([
      "Paketzeile 1",
      "Paketzeile 2",
    ]);
    expect(packageSection.lines.every((l) => l.source.kind === "custom")).toBe(true);

    // Veraltete Revision → Konflikt, keine weitere Revision.
  ...
  it("F1611-DB-02: Duplikat, Archiv, Fremdmandant und Viewer fail-closed", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const templateId = await createPackage(members, { name: "Einmalpaket" });
    ...
    await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => archivePackageTemplate(tx, ctx, {
        schemaVersion: PACKAGE_TEMPLATE_SCHEMA_VERSION,
        id: templateId,
        active: false,
      }),
    ...
    // Fremdmandant sieht die Vorlage nicht.
    const foreign = await createOfferMembers();
    await expect(withAuthorizedTenantOn(
      testPool, foreign.operatorId, foreign.workspaceId,
      (tx, ctx) => applyPackageTemplate(tx, ctx, {
        schemaVersion: PACKAGE_TEMPLATE_SCHEMA_VERSION,
        templateId,
        offerId,
  ...
  it("F1611-DB-03: Leere/überlange/ungültige Zeilen fail-closed", async () => {
    ...
    const create = (lines: unknown[]) => withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => createPackageTemplate(tx, ctx, { ...base, lines: lines as never }),
    );
    await expect(create([])).rejects.toBeInstanceOf(PackageTemplateValidationError);
    await expect(create(Array.from({ length: 51 }, () => ({ ...line })))).rejects.toBeInstanceOf(
      PackageTemplateValidationError,
    );
    // Stück mit halber Einheit.
    await expect(create([{ ...line, quantityMilli: 1_500 }])).rejects.toBeInstanceOf(
      PackageTemplateValidationError,
    );
    await expect(create([{ ...line, displayName: "   " }])).rejects.toBeInstanceOf(
      PackageTemplateValidationError,
    );
    await expect(create([{ ...line, salesUnitNetCents: -1 }])).rejects.toBeInstanceOf(
      PackageTemplateValidationError,
    );
    await expect(create([{ ...line, unit: "liter" }])).rejects.toBeInstanceOf(
      PackageTemplateValidationError,
    );
  });
});
