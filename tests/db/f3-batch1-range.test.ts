...
// planning_string) + F3-05b (0275, planning_string_equipment)
// + F3-04b (0276, planning_panel_deselect)
// + F3-05c (0277, planning_string_member).
//
...
//   in umgekehrter Reihenfolge (0277 vor 0276 vor 0275 vor 0274 vor
//   0273 vor 0272 vor 0271 vor 0270) ausführbar.
// - Range 0270-0279: nur 0270 + 0271 + 0272 + 0273 + 0274 + 0275 + 0276
//   + 0277 belegt, 0278-0279 frei.
// - Journal: idx lückenlos, jede Entry-Tag-Datei existiert,
//   0270/0271/0272/0273/0274/0275/0276/0277 unmittelbar
//   hintereinander (Batch-Atomizität).
...
const RANGE_SIXTH = "0275";
const RANGE_SEVENTH = "0276";
const RANGE_EIGHTH = "0277";
const RANGE_FREE = ["0278", "0279"];
...
const EQUIP_TABLE = "planning_string_equipment";
const DESELECT_TABLE = "planning_panel_deselect";
const MEMBER_TABLE = "planning_string_member";
...
describe("F3-Batch-1+2 Range 0270-0279: nur 0270 + 0271 + 0272 + 0273 + 0274 + 0275 + 0276 + 0277 belegt", () => {
  ...
  it("belegt 0275 (F3-05b planning_string_equipment) mit genau einer Up-Datei", () => {
    expect(requireSingleUp(RANGE_SIXTH)).toMatch(/^0275_.+\.sql$/);
  });

  it("belegt 0276 (F3-04b planning_panel_deselect) mit genau einer Up-Datei", () => {
    expect(requireSingleUp(RANGE_SEVENTH)).toMatch(/^0276_.+\.sql$/);
  });

  it("belegt 0277 (F3-05c planning_string_member) mit genau einer Up-Datei", () => {
    expect(requireSingleUp(RANGE_EIGHTH)).toMatch(/^0277_.+\.sql$/);
  });

  it("laesst 0278-0279 frei (keine Up- oder Down-Dateien)", () => {
...
describe("F3-Batch-1 Journal-Kontinuitaet", () => {
  ...
  it("reiht 0270/0271/0272/0273/0274/0275/0276/0277 unmittelbar hintereinander", () => {
    ...
    const sixth = requireJournalEntry(RANGE_SIXTH);
    const seventh = requireJournalEntry(RANGE_SEVENTH);
    const eighth = requireJournalEntry(RANGE_EIGHTH);
    ...
    expect(sixth.idx, "0275 muss direkt auf 0274 folgen (Batch-Atomizitaet).").toBe(fifth.idx + 1);
    expect(seventh.idx, "0276 muss direkt auf 0275 folgen (Batch-Atomizitaet).").toBe(sixth.idx + 1);
    expect(eighth.idx, "0277 muss direkt auf 0276 folgen (Batch-Atomizitaet).").toBe(seventh.idx + 1);
    const journal = migrationJournal();
    ...
    expect(journal.entries[headPosition + 5]?.tag).toBe(sixth.tag);
    expect(journal.entries[headPosition + 6]?.tag).toBe(seventh.tag);
    expect(journal.entries[headPosition + 7]?.tag).toBe(eighth.tag);
  });
});
...
describe("F3-Batch-1 Up-Pfade je Migration", () => {
  ...
  it("0275-Up erzeugt planning_string_equipment mit FK auf planning_string (F3-05b)", () => {
    const file = requireSingleUp(RANGE_SIXTH);
    const sql = readFileSync(resolve("drizzle", file), "utf8");
    expect(sql.trim().length).toBeGreaterThan(0);
    expect(sql).toMatch(/create\s+table\s+"?planning_string_equipment"?/iu);
    expect(sql).toMatch(/references\s+"?(?:public"\.")?planning_string"?/iu);
  });

  it("0276-Up erzeugt planning_panel_deselect mit FK auf planning_panel_group (F3-04b)", () => {
    const file = requireSingleUp(RANGE_SEVENTH);
    const sql = readFileSync(resolve("drizzle", file), "utf8");
    expect(sql.trim().length).toBeGreaterThan(0);
    expect(sql).toMatch(/create\s+table\s+"?planning_panel_deselect"?/iu);
    expect(sql).toMatch(/references\s+"?(?:public"\.")?planning_panel_group"?/iu);
  });

  it("0277-Up erzeugt planning_string_member mit FKs auf String + Gruppe (F3-05c)", () => {
    const file = requireSingleUp(RANGE_EIGHTH);
    const sql = readFileSync(resolve("drizzle", file), "utf8");
    expect(sql.trim().length).toBeGreaterThan(0);
    expect(sql).toMatch(/create\s+table\s+"?planning_string_member"?/iu);
    expect(sql).toMatch(/references\s+"?(?:public"\.")?planning_string"?\(/iu);
    expect(sql).toMatch(/references\s+"?(?:public"\.")?planning_panel_group"?/iu);
  });
});
...
describe("F3-Batch-1 Down-Pfade je Migration", () => {
  ...
  it("0275-Down existiert als drizzle/<tag>.down.sql und baut planning_string_equipment ab", () => {
    const { tag } = requireJournalEntry(RANGE_SIXTH);
    const downPath = downFileFor(tag);
    expect(existsSync(downPath), `F3-Batch-2 Down-Pfad fehlt: ${downPath} existiert nicht (RED: Datei fehlt).`).toBe(true);
    const sql = readFileSync(downPath, "utf8");
    expect(sql.trim().length).toBeGreaterThan(0);
    expect(sql).toMatch(/drop\s+table[^;]*planning_string_equipment/iu);
  });

  it("0276-Down existiert als drizzle/<tag>.down.sql und baut planning_panel_deselect ab", () => {
    const { tag } = requireJournalEntry(RANGE_SEVENTH);
    const downPath = downFileFor(tag);
    expect(existsSync(downPath), `F3-Batch-2 Down-Pfad fehlt: ${downPath} existiert nicht (RED: Datei fehlt).`).toBe(true);
    const sql = readFileSync(downPath, "utf8");
    expect(sql.trim().length).toBeGreaterThan(0);
    expect(sql).toMatch(/drop\s+table[^;]*planning_panel_deselect/iu);
  });

  it("0277-Down existiert als drizzle/<tag>.down.sql und baut planning_string_member ab", () => {
    const { tag } = requireJournalEntry(RANGE_EIGHTH);
    const downPath = downFileFor(tag);
    expect(existsSync(downPath), `F3-Batch-2 Down-Pfad fehlt: ${downPath} existiert nicht (RED: Datei fehlt).`).toBe(true);
    const sql = readFileSync(downPath, "utf8");
    expect(sql.trim().length).toBeGreaterThan(0);
    expect(sql).toMatch(/drop\s+table[^;]*planning_string_member/iu);
  });
});

describe("F3-Batch-1 Up/Down auf frischer DB", () => {
  it("migriert 0270/0271/0272/0273/0274/0275/0276/0277 auf frischer DB mit RLS + FORCE (Up)", async () => {
    ...
    requireJournalEntry(RANGE_SIXTH);
    requireJournalEntry(RANGE_SEVENTH);
    requireJournalEntry(RANGE_EIGHTH);
    ...
    requireSingleUp(RANGE_SIXTH);
    requireSingleUp(RANGE_SEVENTH);
    requireSingleUp(RANGE_EIGHTH);
    ...
    try {
      ...
      const relations = await pool.query<{
        ...
        `select relname, relrowsecurity, relforcerowsecurity
          ...
          order by relname`,
        [[SOURCE_TABLE, ROOF_TABLE, RESTRICTION_TABLE, PANEL_TABLE, INVERTER_TABLE, STRING_TABLE, EQUIP_TABLE, DESELECT_TABLE, MEMBER_TABLE]],
      );
      expect(
        ...
        { relname: DESELECT_TABLE, relrowsecurity: true, relforcerowsecurity: true },
        ...
        { relname: EQUIP_TABLE, relrowsecurity: true, relforcerowsecurity: true },
        { relname: MEMBER_TABLE, relrowsecurity: true, relforcerowsecurity: true },
      ]);
      // Journal-v7 trackt per id/hash (keine tag-Spalte): voller
      // Migrate-Lauf = Journal-Länge applied; Tabellen-Existenz oben
      // beweist 0270+0271+0272+0273+0274+0275+0276+0277-Anwendung.
  ...
  it("baut 0277 vor 0276 vor 0275 vor 0274 vor 0273 vor 0272 vor 0271 vor 0270 per Down-SQL wieder ab (Down, umgekehrte Reihenfolge)", async () => {
    // Fail-fast ohne DB-Boot, solange die Dateien fehlen (RED-Signatur).
    ...
    const tag0275 = requireJournalEntry(RANGE_SIXTH).tag;
    const tag0276 = requireJournalEntry(RANGE_SEVENTH).tag;
    const tag0277 = requireJournalEntry(RANGE_EIGHTH).tag;
    requireSingleUp(RANGE_HEAD);
    requireSingleUp(RANGE_NEXT);
    requireSingleUp(RANGE_THIRD);
    requireSingleUp(RANGE_FOURTH);
    ...
    const down0275 = downFileFor(tag0275);
    const down0276 = downFileFor(tag0276);
    const down0277 = downFileFor(tag0277);
    expect(existsSync(down0277), `F3-Batch-2 Down-Pfad fehlt: ${down0277} (RED: Datei fehlt).`).toBe(true);
    expect(existsSync(down0276), `F3-Batch-2 Down-Pfad fehlt: ${down0276} (RED: Datei fehlt).`).toBe(true);
    expect(existsSync(down0275), `F3-Batch-2 Down-Pfad fehlt: ${down0275} (RED: Datei fehlt).`).toBe(true);
    ...
    try {
      await migrate(drizzle(pool), { migrationsFolder: drizzleDir() });
      // Down in umgekehrter Reihenfolge: erst Member (0277),
      // dann Deselect, Equipment, Strings/WR, Panel-Gruppe,
      // Restriktion, Dach, Quelle.
      await pool.query(readFileSync(down0277, "utf8"));
      await pool.query(readFileSync(down0276, "utf8"));
      await pool.query(readFileSync(down0275, "utf8"));
