...
describe("F16.3 Slice C Template-Apply global (PostgreSQL)", () => {
  ...
  it("F1603C-DB-02: Cap-Vorlage, Fix-Vorlage und fremde ID werden abgewiesen", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    ...
    const fixId = await createTemplate(members, {
      name: "Fixbetrag",
      kind: "fix_cents",
      amountCents: 1250,
      percentBps: null,
    ...
    await expect(apply(fixId)).rejects.toBeInstanceOf(DiscountTemplateValidationError);
