...
import { archiveDiscountTemplate, createDiscountTemplate } from "@/modules/discounts";
import {
  applyOfferTemplate,
...
import { DiscountTemplateNotFoundError } from "@/modules/discounts";
...
async function createOfferMembers(): Promise<OfferMembers> {
  ...
  await withTenantOn(testPool, members.workspaceId, async (tx) => {
    await tx.execute(sql`
      ...
      values (${members.workspaceId}::uuid, 'F16-06 Angebots-Vorlagen')
    `);
    for (const userId of [
      members.operatorId,
      ...
      await tx.execute(sql`
        ...
        values (${userId}::uuid, ${`${userId}@f1606.test`})
      `);
    }
    await tx.execute(sql`
  ...
  await withTenantOn(testPool, members.workspaceId, async (tx) => {
    await tx.execute(sql`
      insert into contact (
        ...
        'Synthetischer F16-06-Kontakt', 'Fixture', 'Contact', 'f1606.fixture@example.test',
        'f1606.fixture@example.test'
      )
    `);
    await tx.execute(sql`
      insert into site (
        ...
        ${ids.contactId}::uuid, 'Synthetischer F16-06-Standort',
    ...
    await tx.execute(sql`
      ...
      select ${ids.projectId}::uuid, ${members.workspaceId}::uuid,
             ...
             board.id, intake.id, 'Synthetisches F16-06-Projekt', 'wmee-rechner-v3'
...
function productCommand(
  ...
  return {
    ...
    internalSku: `F1606-${type.toUpperCase()}-${index}`,
    ...
    commercial: {
      ...
      purchaseProvenance: {
        ...
        reference: `PRIVATE-F1606-PURCHASE-${type}-${index}`,
      ...
      salesProvenance: {
        ...
        reference: `SYNTHETIC-F1606-SALES-${type}-${index}`,
        observedOn: "2026-08-30",
    ...
    technicalProvenance: {
      ...
      reference: `SYNTHETIC-F1606-TECH-${type}-${index}`,
      observedOn: "2026-08-30",
      rightsBasis: "workspace_owned",
      sourceDocumentSha256: null,
    },
...
async function createPaymentPreset(members: OfferMembers): Promise<string> {
  const created = await withAuthorizedTenantOn(
    testPool, members.operatorId, members.workspaceId,
    (tx, ctx) => createPaymentOption(tx, ctx, {
...
async function createDiscountPreset(members: OfferMembers): Promise<string> {
  const created = await withAuthorizedTenantOn(
    testPool, members.operatorId, members.workspaceId,
    (tx, ctx) => createDiscountTemplate(tx, ctx, {
      schemaVersion: DISCOUNT_TEMPLATE_SCHEMA_VERSION,
      name: "F1606-Fuenf-Prozent",
      kind: "percent_bps",
      amountCents: null,
      percentBps: 500,
      capCents: null,
...
async function createOfferPreset(
  members: OfferMembers,
  input: { name: string; paymentOptionId?: string | null; discountTemplateId?: string | null },
): Promise<string> {
  const created = await withAuthorizedTenantOn(
    testPool, members.operatorId, members.workspaceId,
    (tx, ctx) => createOfferTemplate(tx, ctx, {
...
async function readVariantState(
  ...
  variantId: string,
  revision: number,
): Promise<{ bps: number | null; paymentOptionId: string | null }> {
  return withTenantOn(testPool, workspaceId, async (tx) => {
    const result = await tx.execute<{ bps: number | null; payment_option_id: string | null }>(sql`
      select (revision_snapshot ->> 'globalDiscountBps')::integer as bps,
             (select payment_option_id from offer_variant
               where workspace_id = ${workspaceId}::uuid
                 and offer_id = ${offerId}::uuid
                 and id = ${variantId}::uuid) as payment_option_id
    ...
    return { bps: row.bps, paymentOptionId: row.payment_option_id };
  });
}

describe("F16-06 Angebots-Vorlagen (PostgreSQL)", () => {
  it("F1606-DB-01: Anlage → Liste (canWrite je Rolle) → Anwenden setzt Zahlart + Global-Rabatt", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const paymentOptionId = await createPaymentPreset(members);
    const discountTemplateId = await createDiscountPreset(members);
    const templateId = await createOfferPreset(members, {
      name: "Standard Kauf + 5 %",
      paymentOptionId,
      discountTemplateId,
    });
    ...
    const created = await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => listOfferTemplates(tx, ctx).then((rows) => rows.find((row) => row.id === templateId)!),
    );
    expect(created.name).toBe("Standard Kauf + 5 %");
    expect(created.paymentOptionId).toBe(paymentOptionId);
    expect(created.discountTemplateId).toBe(discountTemplateId);
    ...
    const viewerList = await withAuthorizedTenantOn(
      ...
      (tx, ctx) => listOfferTemplates(tx, ctx),
    );
    ...
    const applied = await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => applyOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        templateId,
        offerId,
        variantId,
    ...
    expect(applied.discountApplied).toBe(true);
    expect(applied.paymentOptionApplied).toBe(true);
    expect(applied.revision).toBe(2);

    const state = await readVariantState(members.workspaceId, offerId, variantId, 2);
    expect(state.bps).toBe(500);
    expect(state.paymentOptionId).toBe(paymentOptionId);

    // Veraltete Revision mit Rabatt-Anteil → Konflikt, keine zweite Revision.
  ...
  it("F1606-DB-02: Duplikat → Konflikt; leere/fremde Presets fail-closed", async () => {
    const { members } = await createBasisOffer();
    const paymentOptionId = await createPaymentPreset(members);
    await createOfferPreset(members, { name: "Doppelt", paymentOptionId });
    await expect(createOfferPreset(members, { name: "  DOPPELT ", paymentOptionId }))
      .rejects.toBeInstanceOf(OfferTemplateConflictError);
    ...
    )).rejects.toBeInstanceOf(OfferTemplateValidationError);

    // Unbekannte Preset-Referenzen.
    await expect(createOfferPreset(members, {
      name: "Fremde Zahlart",
      paymentOptionId: randomUUID(),
    })).rejects.toBeInstanceOf(OfferTemplateValidationError);
    await expect(createOfferPreset(members, {
      name: "Fremder Rabatt",
      discountTemplateId: randomUUID(),
    })).rejects.toBeInstanceOf(OfferTemplateValidationError);

    // Mandantenfremde Referenzen (zweiter Workspace).
    const other = await createOfferMembers();
    const foreignPayment = await createPaymentPreset(other);
    await expect(createOfferPreset(members, {
      name: "Fremder Mandant",
      paymentOptionId: foreignPayment,
    })).rejects.toBeInstanceOf(OfferTemplateValidationError);
  });

  it("F1606-DB-03: Viewer fail-closed; Archiv sperrt Anwenden; Restore hebt auf; stale Revision → Konflikt", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const paymentOptionId = await createPaymentPreset(members);
    const templateId = await createOfferPreset(members, { name: "Nur Zahlart", paymentOptionId });
    ...
    const viewerApply = () => withAuthorizedTenantOn(
      testPool, members.viewerId, members.workspaceId,
      (tx, ctx) => applyOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        templateId,
    ...
    // Viewer scheitert am Vorlagen-Schreibschutz (keine neuen Permissions).
    await expect(viewerApply()).rejects.toBeInstanceOf(PermissionDeniedError);

    const archived = await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => archiveOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        id: templateId,
        active: false,
      }),
    ...
    )).rejects.toBeInstanceOf(OfferTemplateNotFoundError);

    const restored = await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => restoreOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        id: templateId,
        active: true,
      }),
    ...
    // Zahlart-only wendet ohne Revisionsbump an (Revision bleibt 1).
    const applied = await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => applyOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
    ...
    expect(applied.paymentOptionApplied).toBe(true);
    expect(applied.discountApplied).toBe(false);
    const state = await readVariantState(members.workspaceId, offerId, variantId, 1);
    expect(state.paymentOptionId).toBe(paymentOptionId);
  });

  it("F1606-DB-04: unbekannte/fremde Vorlage → NotFound; archivierte Referenzen nicht anwendbar", async () => {
    const { members, offerId, variantId } = await createBasisOffer();
    const paymentOptionId = await createPaymentPreset(members);
    const discountTemplateId = await createDiscountPreset(members);
    const apply = (templateId: string, revision = 1) => withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => applyOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        templateId,
    ...
    await expect(apply(randomUUID())).rejects.toBeInstanceOf(OfferTemplateNotFoundError);

    const other = await createOfferMembers();
    const foreignPayment = await createPaymentPreset(other);
    const foreignTemplate = await withAuthorizedTenantOn(
      testPool, other.operatorId, other.workspaceId,
      (tx, ctx) => createOfferTemplate(tx, ctx, {
        schemaVersion: OFFER_TEMPLATE_SCHEMA_VERSION,
        name: "Fremde Vorlage",
        paymentOptionId: foreignPayment,
    ...
    await expect(apply(foreignTemplate.id)).rejects.toBeInstanceOf(OfferTemplateNotFoundError);

    // Archivierte Zahlart ist referenzierbar, aber nicht anwendbar.
    const archivedPaymentTemplate = await createOfferPreset(members, {
      name: "Archiv-Zahlart",
      paymentOptionId,
    });
    await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => archivePaymentOption(tx, ctx, paymentOptionId),
    );
    await expect(apply(archivedPaymentTemplate)).rejects.toBeInstanceOf(OfferValidationError);

    // Inaktiver Rabatt ist referenzierbar, aber nicht anwendbar.
    const staleTemplate = await createOfferPreset(members, {
      name: "Stiller Rabatt",
      discountTemplateId,
    });
    await withAuthorizedTenantOn(
      testPool, members.operatorId, members.workspaceId,
      (tx, ctx) => archiveDiscountTemplate(tx, ctx, discountTemplateId),
    );
    await expect(apply(staleTemplate)).rejects.toBeInstanceOf(DiscountTemplateNotFoundError);
  });
});
