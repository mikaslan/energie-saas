...
import { REST_INTAKE_SCOPE } from "./types";

export const REST_INTAKE_PATH = "/api/inbound/rest/v1" as const;
export const REST_SIGNATURE_WINDOW_SECONDS = 300;
...
type RestCredentialConfig = {
  keyId: string;
  workspaceId: string;
  scope: typeof REST_INTAKE_SCOPE;
  secretBase64: string;
};

const verifiedBrand: unique symbol = Symbol("VerifiedRestIdentity");

export type VerifiedRestIdentity = {
  readonly keyId: string;
  readonly workspaceId: string;
  readonly scope: typeof REST_INTAKE_SCOPE;
  readonly actor: `api:${string}`;
  readonly [verifiedBrand]: true;
};

export type RestSignatureHeaders = {
  keyId: string | null;
...
export type RestSignatureInput = {
  method: string;
  path: string;
  body: Uint8Array;
  headers: RestSignatureHeaders;
  nowSeconds?: number;
...
function parseCredentials(raw: string | undefined): Array<RestCredentialConfig & { secret: Buffer }> {
  if (!raw) throw new RestCredentialConfigurationError("REST_INTAKE_KEYS_JSON fehlt");

  let value: unknown;
  try {
    value = JSON.parse(raw);
  } catch {
    throw new RestCredentialConfigurationError("JSON ist ungueltig");
  }
  if (!Array.isArray(value) || value.length < 1 || value.length > 20) {
    throw new RestCredentialConfigurationError("erwartet werden 1 bis 20 Keys");
  }

  const seen = new Set<string>();
  return value.map((entry, index) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      throw new RestCredentialConfigurationError(`Key ${index + 1} ist kein Objekt`);
    }
    ...
    if (names.length !== expected.length || names.some((name, i) => name !== expected[i])) {
      throw new RestCredentialConfigurationError(`Key ${index + 1} hat unbekannte oder fehlende Felder`);
    }
    const { keyId, workspaceId, scope, secretBase64 } = record;
    if (typeof keyId !== "string" || !KEY_ID.test(keyId) || seen.has(keyId)) {
      throw new RestCredentialConfigurationError(`Key ${index + 1} hat keine eindeutige gueltige keyId`);
    }
    if (typeof workspaceId !== "string" || !WORKSPACE_UUID.test(workspaceId)) {
      throw new RestCredentialConfigurationError(`Key ${index + 1} hat keine gueltige workspaceId`);
    }
    if (scope !== REST_INTAKE_SCOPE) {
      throw new RestCredentialConfigurationError(`Key ${index + 1} hat einen unzulaessigen Scope`);
    }
    if (typeof secretBase64 !== "string") {
      throw new RestCredentialConfigurationError(`Key ${index + 1} hat kein Secret`);
    }
    const secret = decodeCanonicalBase64(secretBase64);
    if (!secret || secret.length < 32 || secret.length > 64) {
      throw new RestCredentialConfigurationError(`Key ${index + 1} braucht 32 bis 64 Zufallsbytes`);
    }
    seen.add(keyId);
...
export function verifyRestSignature(input: RestSignatureInput): VerifiedRestIdentity {
  const credentials = parseCredentials(input.credentialsJson ?? process.env.REST_INTAKE_KEYS_JSON);
  ...
  const timeIsFresh = validTimestamp && Number.isSafeInteger(requestTime)
    && Math.abs(now - requestTime) <= REST_SIGNATURE_WINDOW_SECONDS;
  const authenticated =
    input.method === "POST"
    && input.path === REST_INTAKE_PATH
    && validKeyId
    && validIdempotency
    && validHash
    && canonicalSignature
  ...
  if (!authenticated || !credential) throw new RestAuthenticationError();
