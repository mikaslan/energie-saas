...
// F16.3 Slice A Rabatt-Vorlagen — interner DTO-/Command-Vertrag.
...
export const DISCOUNT_TEMPLATE_SCHEMA_VERSION = 1;

export const DISCOUNT_TEMPLATE_NAME_MAX = 200;

export const DISCOUNT_KIND_FIX = "fix_cents" as const;
export const DISCOUNT_KIND_PERCENT = "percent_bps" as const;

const cleanName = z
  .string()
  .transform((v) => v.normalize("NFKC").trim())
  .refine((v) => v.length >= 1 && v.length <= DISCOUNT_TEMPLATE_NAME_MAX, { message: "ungültige Länge" })
...
const kindFields = {
  kind: z.enum([DISCOUNT_KIND_FIX, DISCOUNT_KIND_PERCENT]),
...
// symmetrisch zu den DB-CHECKs discount_template_{fix,percent,cap}_ck.
function checkKindConsistency(value: {
  ...
  if (value.kind === DISCOUNT_KIND_FIX) {
...
export const discountTemplateDtoSchema = z.object({
  schemaVersion: z.literal(DISCOUNT_TEMPLATE_SCHEMA_VERSION),
  id: z.string().uuid(),
  name: z.string(),
  kind: z.enum([DISCOUNT_KIND_FIX, DISCOUNT_KIND_PERCENT]),
...
export type DiscountTemplateDto = z.infer<typeof discountTemplateDtoSchema>;

export const createDiscountTemplateCommandSchema = z
  .object({
    schemaVersion: z.literal(DISCOUNT_TEMPLATE_SCHEMA_VERSION),
    name: cleanName,
    ...kindFields,
...
export type CreateDiscountTemplateCommand = z.infer<typeof createDiscountTemplateCommandSchema>;

export const updateDiscountTemplateCommandSchema = z
  .object({
    schemaVersion: z.literal(DISCOUNT_TEMPLATE_SCHEMA_VERSION),
    id: z.string().uuid(),
    name: cleanName,
  ...
  .refine(checkKindConsistency, { message: "kind und Beträge passen nicht zusammen" });
export type UpdateDiscountTemplateCommand = z.infer<typeof updateDiscountTemplateCommandSchema>;
