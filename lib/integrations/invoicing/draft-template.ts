...
type DraftLine = DraftPdfInputV1["lines"][number];

function renderLine(line: DraftLine): string {
  return `<tr>
...
function documentKindLabel(input: DraftPdfInputV1): string {
...
function renderSenderAddress(input: DraftPdfInputV1): string {
...
function renderRecipientAddress(input: DraftPdfInputV1): string {
  const recipient = input.recipient;
  if (recipient === null) {
    return `<p>Kein Empfänger hinterlegt</p>`;
  }
  ...
  const lines = [
    ...
    street,
    recipient.postalCode === null || recipient.city === null
      ? null
...
/**
 * F8-24c Draft-Vorschau: eigener reduzierter Entwurf (Kopf/Zeilen/Summen),
 * fettes ENTWURF-Wasserzeichen je Seite, keine Nummern-/Ausstellungs-Zeile,
 * keine Siegel-/Hash-Aussage. Pure deterministische HTML-Ausgabe.
 */
export function renderDraftPdfHtml(value: DraftPdfInputV1): string {
  const validated = validateDraftPdfInput(value);
  if (!validated.ok) {
    throw new TypeError("Ungueltiger Draft-PDF-Input.");
  }
  const input = validated.value;
  const kind = documentKindLabel(input);
  const standDate = formatDate(input.preparedAt.slice(0, 10));
  const pageFooterText = `Entwurf ${input.document.name} · Stand ${standDate}`;
...
<head>
  ...
  <title>Entwurf ${escapeHtml(input.document.name)}</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .draft-watermark { position: fixed; inset: 0; display: flex; align-items: center; justify-content: center; pointer-events: none; z-index: 999; }
    .draft-watermark span { display: block; transform: rotate(-32deg); font-size: 120pt; font-weight: 900; letter-spacing: 0.08em; color: rgba(11, 27, 21, 0.10); white-space: nowrap; }
    .draft-badge { display: inline-block; margin-top: 2mm; padding: 1mm 4mm; border: 1mm solid #b45309; border-radius: 2mm; color: #92400e; font-weight: 800; letter-spacing: 0.10em; }
    ...
    .draft-hint { margin-top: 7mm; padding: 4mm; border: 0.35mm solid #b45309; background: #fffbeb; break-inside: avoid; page-break-inside: avoid; }
    .draft-hint p:last-child { margin-bottom: 0; }
...
<body>
  <div class="draft-watermark" aria-hidden="true"><span>ENTWURF</span></div>
  <header class="document-header">
    <div>
      <div class="wordmark">${escapeHtml(input.sender.companyName)}</div>
      <div class="document-kind">${escapeHtml(kind)} (Entwurf)</div>
      <div class="draft-badge">ENTWURF</div>
    </div>
    <div class="document-number"><span>Belegname</span><strong>${escapeHtml(input.document.name)}</strong></div>
  </header>
  <main>
    <h1>${escapeHtml(kind)} – Entwurf</h1>
    ...
    <dl class="meta-grid">
      <div><dt>Stand</dt><dd>${escapeHtml(standDate)}</dd></div>
...
${input.lines.map(renderLine).join("\n")}
    ...
    <aside class="draft-hint" aria-label="Entwurfshinweis">
      <p><strong>Unverbindliche Vorschau.</strong> Dieser Entwurf ist keine Rechnung und kein Angebot — Nummern und Daten werden erst bei Ausstellung vergeben.</p>
    </aside>
  </main>
