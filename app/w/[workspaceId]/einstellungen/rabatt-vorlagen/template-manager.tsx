...
import {
  archiveDiscountTemplateAction,
  createDiscountTemplateAction,
  type DiscountTemplateActionState,
  restoreDiscountTemplateAction,
  updateDiscountTemplateAction,
} from "./actions";

const initialState: DiscountTemplateActionState = { status: "idle" };

function Feedback({ state }: { state: DiscountTemplateActionState }) {
...
function formatValue(template: DiscountTemplateDto): string {
...
// F16.3 Slice A: Create-/Edit-Formular (kind-Umschalter: Fix braucht
...
function TemplateForm({
  ...
  workspaceId: string;
  template?: DiscountTemplateDto;
  action: (
    previous: DiscountTemplateActionState,
    formData: FormData,
...
export function DiscountTemplateManager({
  workspaceId,
  templates,
  canWrite,
}: {
  workspaceId: string;
  templates: DiscountTemplateDto[];
  canWrite: boolean;
}) {
  const [createState] = useActionState(createDiscountTemplateAction, initialState);
  const [archiveState, archiveDispatch] = useActionState(archiveDiscountTemplateAction, initialState);
  const [restoreState, restoreDispatch] = useActionState(restoreDiscountTemplateAction, initialState);
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-3">
            <TemplateForm
              ...
              action={createDiscountTemplateAction}
              submitLabel="Anlegen"
            />
          </div>
        </section>
                ...
                {editingId === template.id && canWrite ? (
                  <div className="mt-3 border-t border-slate-100 pt-3">
                    <TemplateForm
                      workspaceId={workspaceId}
                      template={template}
                      action={updateDiscountTemplateAction}
