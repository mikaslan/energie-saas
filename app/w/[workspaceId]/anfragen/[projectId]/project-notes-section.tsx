...
function NoteCard({
  ...
  return (
    <article id={`project-note-${note.id}`} className={`min-w-0 scroll-mt-6 rounded-lg border bg-white p-4 shadow-sm ${note.pinned ? "border-brand-300 ring-1 ring-brand-100" : "border-slate-200"}`}>
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          {note.pinned ? (
            <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-brand-100 px-2 py-1 text-xs font-semibold text-brand-900">
        ...
        <div className="mt-4 flex flex-wrap items-start gap-2 border-t border-slate-200 pt-4">
          <form action={action}>
            ...
            <LocalSubmitButton
              ...
              className="min-h-11 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-800 outline-none hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-brand-600 focus-visible:ring-offset-2 disabled:cursor-wait disabled:text-slate-400"
            >
