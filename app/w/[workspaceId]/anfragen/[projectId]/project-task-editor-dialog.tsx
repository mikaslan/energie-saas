...
import {
  ...
  loadTaskTeamAssignment,
  type TaskTeamAssignmentActionState,
  type TaskTeamAssignmentLoadState,
} from "./task-team-assignment-actions";
...
const INITIAL_TASK_TEAM_STATE: TaskTeamAssignmentActionState = { status: "idle" };

function taskTeamMutationMessage(state: TaskTeamAssignmentActionState): string {
  switch (state.status) {
    case "success":
    ...
    case "invalid":
      return "Die Änderung war unvollständig oder ungültig. Bitte schließe den Dialog und öffne die Aufgabe erneut.";
    case "conflict":
      return "Die Zuweisung wurde zwischenzeitlich geändert. Die Ansicht wurde aktualisiert.";
    case "target_unavailable":
      return "Dieses Team ist für den Arbeitsbereich nicht mehr verfügbar.";
    case "limit_reached":
      return "Die Aufgabe hat bereits die maximale Anzahl Teamzuweisungen erreicht.";
    case "not_found":
      return "Die Aufgabe ist nicht mehr verfügbar.";
    case "archived":
      return "Archivierte Aufgaben können nicht mehr bearbeitet werden.";
    case "denied":
...
function TaskTeamCommandFields({
  commandVersion,
  kind,
  taskId,
...
function TaskTeamAssignmentSection({
  workspaceId,
  projectId,
  taskId,
}: {
  ...
  const [loaded, setLoaded] = useState<TaskTeamAssignmentLoadState | null>(null);
  const [mutationState, mutationAction, mutationPending] = useActionState(
    changeTaskTeamAssignment.bind(null, workspaceId, projectId),
    INITIAL_TASK_TEAM_STATE,
  );
  const [, startLoadTransition] = useTransition();
  const feedbackRef = useRef<HTMLParagraphElement | null>(null);
  const message = taskTeamMutationMessage(mutationState);
  const isError = mutationState.status !== "idle" && mutationState.status !== "success";

  const reload = useCallback(() => {
    startLoadTransition(async () => {
      setLoaded(await loadTaskTeamAssignment(workspaceId, projectId, taskId));
    });
  ...
  useEffect(() => {
    ...
    reload();
  }, [mutationState, reload]);

  if (loaded === null) {
    return (
      <section id="task-team-assignment" aria-labelledby="task-team-assignment-title" className="mt-6 min-w-0 rounded-md border border-slate-200 p-4">
        <h2 id="task-team-assignment-title" className="text-sm font-semibold text-slate-900">
          Teams
        </h2>
    ...
    return (
      <section id="task-team-assignment" aria-labelledby="task-team-assignment-title" className="mt-6 min-w-0 rounded-md border border-slate-200 p-4">
        ...
        <p role="alert" className="mt-2 text-sm font-semibold text-red-800">
          Die Teamzuweisung konnte nicht geladen werden. Bitte schließe den Dialog und öffne die Aufgabe erneut.
        </p>
      </section>
    );
  }

  const assignment = loaded.context;
  const teamOptions: TeamOption[] = loaded.teamOptions;
              ...
              <li key={team.teamId} className="flex min-w-0 flex-wrap items-center justify-between gap-3 rounded-md border border-slate-200 px-3 py-2">
                ...
                {assignment.canAssign ? (
                  <form action={mutationAction}>
                    <TaskTeamCommandFields
                      commandVersion={loaded.commandVersion}
                      kind="unassign_team"
                      taskId={taskId}
                    ...
                    <button
                      ...
                      aria-label={`${team.label} von der Aufgabe entfernen`}
        ...
        <div className="mt-5 min-w-0 border-t border-slate-200 pt-5">
          ...
          {assignableOptions.length === 0 ? (
            ...
            <form action={mutationAction} className="mt-3 grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
              ...
              <input type="hidden" name="schemaVersion" value={loaded.commandVersion} />
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-brand-800">Aufgabe</p>
