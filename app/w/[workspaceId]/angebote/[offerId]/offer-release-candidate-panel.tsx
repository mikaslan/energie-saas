...
const fieldClass = "mt-1 min-h-11 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-base text-slate-950 outline-none focus-visible:border-brand-600 focus-visible:ring-2 focus-visible:ring-brand-600/30 disabled:cursor-not-allowed disabled:bg-slate-100";
...
function CheckRow({
  ...
  const effectiveInvalid = invalid && invalidClearedFor !== validationState;
  return (
    <label htmlFor={id} className={`flex min-h-11 cursor-pointer items-start gap-3 rounded-md border bg-white px-3 py-3 text-sm leading-6 text-slate-800 focus-within:ring-2 focus-within:ring-brand-600 focus-within:ring-offset-2 ${effectiveInvalid ? "border-rose-600" : "border-slate-200"}`}>
      <input
        id={id}
        name={name}
        value="true"
