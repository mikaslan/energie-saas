...
export function ImportControls({
  ...
  return (
    <section aria-labelledby="catalog-import-control-title" className="rounded-lg border border-brand-200 bg-brand-50 p-5 sm:p-6">
      <h2 id="catalog-import-control-title" className="text-lg font-semibold text-brand-950">Importentscheidung</h2>
      ...
      {validCount > 0 ? (
        <form action={startAction} onSubmit={() => setLastOperation("start")} className="mt-5 grid gap-4">
          ...
          <label className="flex items-start gap-3 rounded-md border border-brand-300 bg-white p-4 text-sm leading-6 text-slate-800">
            <input type="checkbox" name="rightsAttested" value="yes" required disabled={startPending || cancelPending} className="mt-1 size-4 shrink-0 accent-brand-700" />
            <span>{CATALOG_IMPORT_RIGHTS_ATTESTATION_TEXT}</span>
          </label>
          <button type="submit" aria-disabled={startPending || cancelPending || undefined} disabled={startPending || cancelPending} className="min-h-11 w-full rounded-md bg-brand-700 px-4 text-sm font-semibold text-white outline-none hover:bg-brand-800 focus-visible:ring-2 focus-visible:ring-brand-600 focus-visible:ring-offset-2 disabled:cursor-wait disabled:bg-slate-500 sm:w-fit">
