...
// F7-02E (0142): validBlocks erneut geerntet (text-Art + value-Spiegel);
// Ist-Wert aus Gate-Lauf nach 0142-Anwendung, Gate verifiziert erneut.
// F7-02F (0143): validBlocks erneut geerntet (multi-Art, keine neue
...
// F7-02I (0175): validBlocks erneut geerntet (signature-Art +
// signerRole-Spiegel, photo-Spiegel erweitert); checklistStructure
// unveraendert (Rolle = Struktur, Bytes = Nutzlast).
...
// F7-15 (0180): BEIDE Pins neu geerntet (photos-Array in Allowlist +
// Array-Regel; photos im Struktur-Strip — Methode vorab gegen die
// 0179-Pins gegengeprueft, beide exakt reproduziert).
// F7-02L (0185): validBlocks erneut geerntet (circuit-plan-Art in
// kind-IN + Anzeige-Regel); checklistStructure unveraendert (Art ohne
// Nutzlast, kein Strip — 0180-prosrc kalibriert exakt auf alten Pin,
// 0185-Live-prosrc bestaetigt den neuen Pin).
const F704_CHECKLIST_FUNCTION_SHA256 = {
  ...
  checklistStructure: "b5e3d3262855b14c62537a030adf3a203461707be3cd5f2ead8d956d709c09f3",
  ...
  validBlocks: "39b886ec9e63f10bdf455aaf7ccda804724f9c37ea4f43be80bca9626927f165",
} as const;
...
const APPOINTMENT_TEMPLATE_RELATIONS = [
...
// F8-01 (0083): eigene Menge — alte Prefixe (z. B. m204-Test mit 0..75)
// kennen die Link-Tabelle nicht; die M3-01-Kernmenge bleibt stabil.
...
export async function applyRoleContract(client: PoolClient): Promise<void> {
  ...
  // F16-05: Termin-Vorlagen — Archiv statt Delete (kein DELETE-Grant).
  const hasAppointmentTemplates = await hasAtomicPublicRelationSet(
    client,
    APPOINTMENT_TEMPLATE_RELATIONS,
    "Rollen-ACL-Manifest: F16-05-Termin-Vorlagen",
  );
  if (hasAppointmentTemplates) {
    await client.query(`
      revoke all privileges on
        public.appointment_template
        from public, app_migrator, app_runtime, app_system, app_auth,
          app_worker, app_erasure, app_membership_writer, identity_reconciler;
      grant select, insert, update on public.appointment_template to app_runtime
    `);
  }
  ...
  const hasPortalStatusLabels = await hasAtomicPublicRelationSet(
    ...
    "Rollen-ACL-Manifest: F10-05-Portal-Statusmapping",
  );
  if (hasPortalStatusLabels) {
    await client.query(`
      revoke all privileges on
        public.portal_status_label
        from public, app_migrator, app_runtime, app_system, app_auth,
          app_worker, app_erasure, app_membership_writer, identity_reconciler;
      grant select, insert, update, delete on public.portal_status_label to app_runtime
    `);
  }
  ...
  const hasServiceCasesForAcl = await hasAtomicPublicRelationSet(
    client,
    SERVICE_CASE_RELATIONS,
    "Rollen-ACL-Manifest: F13-01-Serviceauftrag",
  );
  if (hasServiceCasesForAcl) {
    await client.query(`
      revoke all privileges on
        public.service_case
        from public, app_migrator, app_runtime, app_system, app_auth,
          app_worker, app_erasure, app_membership_writer, identity_reconciler;
      grant select, insert, update on public.service_case to app_runtime
    `);
  }
  ...
  // F8-01 (0083): eigene ACL-Menge — alte Prefixe kennen die Tabelle nicht.
  const hasCommercialDocumentLinksForAcl = await hasAtomicPublicRelationSet(
    client,
    COMMERCIAL_DOCUMENT_LINK_RELATIONS,
    "Rollen-ACL-Manifest: F8-01-Anzahlung-Link",
  );
  if (hasCommercialDocumentLinksForAcl) {
    await client.query(`
      revoke all privileges on
  ...
  // F7-16: Projekt-Dateien — Anlage + Lesen (Muster subsidy_case_message).
  // F10-17: + UPDATE nur fuers Sichtbarkeits-Flag (Toggle; Bytes/Key bleiben
  // immutabel, kein Delete). Ohne UPDATE schlaegt der Toggle mit
  // permission-denied fehl (F10-17-E2E-Befund).
  ...
  if (hasProjectFilesForAcl) {
    await client.query(`
      revoke all privileges on
        ...
        from public, app_migrator, app_runtime, app_system, app_auth,
          app_worker, app_erasure, app_membership_writer, identity_reconciler;
      grant select, insert, update on public.project_file to app_runtime
    `);
  }
...
export async function verifyRoleContract(
  ...
  // F10-03c (0098): Stufenmarker für project.scope im Portal-Resolver
  // (Muster 0097: Marker wählt den exakten Pin, kein Selbstabgleich).
  ...
  // F10-05 (0113): Stufenmarker für statusLabels im Portal-Resolver
  // (Muster 0109).
  ...
  // F10-14 (0171): Stufenmarker für statusVisibility im Portal-Resolver
  // (Muster 0135).
  ...
  // F10-16 (0179): Stufenmarker für handover_countersigned in der
  // Timeline-Allowlist des Portal-Resolvers (Muster 0172).
  const hasPortalTimelineCountersigned = portalResolverProbe.rows.some(
    (row) => typeof row.source === "string" && row.source.includes("installation.handover_countersigned"),
  );
  // F10-17 (0182): Stufenmarker für projectFiles im Portal-Resolver
  // (Muster 0179).
  ...
  // F10-17 (0182): Stufenmarker für den Portal-Datei-Download
  // (eigene DEFINER-Funktion, Muster 0116).
  const portalProjectFileDownloadProbe = await client.query<{ name: string | null }>(`
    select routine.proname as name
      from pg_catalog.pg_proc as routine
      join pg_catalog.pg_namespace as namespace
        on namespace.oid = routine.pronamespace
  ...
  // F8-01 (0083): eigene Gate-Menge — alte Prefixe (m204-Test mit 0..75)
  // duerfen die Link-Tabelle nicht kennen.
  const hasCommercialDocumentLinks = await hasAtomicPublicRelationSet(
    client,
    COMMERCIAL_DOCUMENT_LINK_RELATIONS,
  ...
  const hasServiceCases = await hasAtomicPublicRelationSet(
    client,
    SERVICE_CASE_RELATIONS,
  ...
  equalRows(
    ...
    [
      ...
      // F10-17 (0182): Portal-Datei-Kapsel (Muster issuance-Kapsel).
      ...(hasPortalProjectFileDownload ? [
        "read_portal_project_file_artifact:app_owner",
      ] : []),
      ...(hasPortalService ? [
      ...
      ...(hasPortal ? [
        ...
        "resolve_portal_public_view(bytea):jsonb:app_owner:plpgsql:f:v:true:false:false:u:" +
          ...
          `search_path=pg_catalog:${hasPortalProjectFileWithdrawnProjection
            ...
            : hasPortalFileRequestSubsidyLinked
  ...
  equalRows(
    ...
    [
      ...
      ...(hasCommercialDocumentLinks ? COMMERCIAL_DOCUMENT_LINK_RELATIONS.flatMap((relation) => [
        `app_runtime:${relation}:INSERT:app_owner:false`,
        `app_runtime:${relation}:SELECT:app_owner:false`,
        `app_runtime:${relation}:UPDATE:app_owner:false`,
        `app_runtime:${relation}:DELETE:app_owner:false`,
      ...
      // F7-16: Anlage/Lesen (Muster subsidy_case_message).
      // F10-17: + UPDATE nur fuers Sichtbarkeits-Flag (Toggle).
  ...
  equalRows(
    ...
    [
      ...
      // F10-17 (0182): Portal-Datei-Kapsel (Muster issuance-Kapsel).
      ...(hasPortalProjectFileDownload ? [
        "app_runtime:read_portal_project_file_artifact(bytea, uuid):EXECUTE:app_owner:false",
      ] : []),
      ...(hasPortalService ? [
